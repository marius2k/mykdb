          ...
          <td width="20%" align="center">
            <form method="post" style="display:inline;">
              <input type="hidden" name="comment_id" value="<?= $c['id'] ?>">
                ...
                ?>
                        ...
                        <?php if ($c['status'] == 'pending' || $c['status'] == 'rejected'){ ?>
                            <a href="?approve=<?= $c['id']?>" ><img class="op-icon" src="<?=APP_URL?>assets/icons/icon-approve.svg" title="<?= lang('lang_btn_approve') ?>"></a>
                        <?php } ?>
                <?php } ?>
                ...
                <?php
                    ...
                    if (hasPermission($_SESSION['user']['id'],$op)){
                ?>  
                        <a href="edit_comment.php?cid=<?= $c['id'] ?>"><img class="op-icon" src="<?=APP_URL?>assets/icons/icon-edit.svg" title="<?= lang('lang_btn_edit') ?>"></a>
                ...
                ?>
                    ...
                    <?php if ( $c['status'] == 'approved'){ ?>
                        <a href="?reject=<?= $c['id'] ?>"><img class="op-icon" src="<?=APP_URL?>assets/icons/icon-reject.svg" title="<?= lang('lang_btn_reject') ?>"></a>
                    <?php } ?>
              ...
              <?php
                    ...
                    if (hasPermission($_SESSION['user']['id'],$op)){
                ?>
                        <a href="?delete=<?= $c['id'] ?>" onclick="return confirm('Sigur?')"><img class="op-icon" src="<?=APP_URL?>assets/icons/icon-delete.svg" title="<?= lang('lang_btn_delete') ?>"></a>
                <?php } ?>
