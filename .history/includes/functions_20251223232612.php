...
function getTop5ViewedArticles(int $days): string 
{
    ...
    try {
        ...
        foreach ($results as $row) {
            ...
            $titleShort = truncateText($row['title'], 50, '...');
...
function getTop5CommentedArticles(int $days): string 
{
    ...
    try {
        ...
        foreach ($results as $article) {
            $titleShort = truncateText($article['title'], 35, '...');
...
// Get trending articles based on recent activity (views, likes, comments in last 7 days)
function getTrendingArticles(int $limit = 5, int $days = 7): string 
{
    if ($limit <= 0 || $days <= 0) {
        return '<p>Parameters must be positive integers.</p>';
    }

    $db = new Database();
    
    // Calculate trending score: recent views + (likes * 2) + (comments * 3)
    // Higher weight for engagement (likes/comments) vs passive views
    $sql = "SELECT 
                a.id, 
                a.title, 
                c.icon,
                (
                    ...
                    (COALESCE(COUNT(DISTINCT ac.id), 0) * 3)
                ) AS trend_score
            FROM articles a
            JOIN categories c ON a.category_id = c.id
            LEFT JOIN article_views v ON a.id = v.article_id 
                AND v.viewed_at >= DATE_SUB(NOW(), INTERVAL :days1 DAY)
            LEFT JOIN article_likes l ON a.id = l.article_id 
                AND l.vote_type = 'like'
                AND l.created_at >= DATE_SUB(NOW(), INTERVAL :days2 DAY)
            LEFT JOIN article_comments ac ON a.id = ac.article_id 
                AND ac.status = 'approved'
                AND ac.created_at >= DATE_SUB(NOW(), INTERVAL :days3 DAY)
            WHERE a.status = 'published'
              AND (a.publish_at IS NULL OR a.publish_at <= NOW())
              AND c.is_active = 1
            GROUP BY a.id, a.title, c.icon
    ...
    try {
        ...
        if (!$results) return '<p>No trending articles at the moment.</p>';

        $html = '<ul class="top-articles trending">';
        foreach ($results as $row) {
            $iconPath = APP_URL . 'assets/icons/categories/' . htmlspecialchars($row['icon']);
            $titleShort = truncateText($row['title'], 35, '...');
            $trendIcon = APP_URL . 'assets/icons/icon-trend.svg';
            $html .= '<li class="grid-li">
                        <span><img src="' . $iconPath . '" class="li-icon" alt=""></span>
                        <span><a href="view_article.php?id=' . $row['id'] . '">' . htmlspecialchars($titleShort) . '</a></span>
                        <span class="span-1 trending-badge"><img src="' . $trendIcon . '" class="trend-icon" alt="trending"></span>
                      </li>';
        }
        $html .= '</ul>';
        return $html;
...
function getRecentArticles(int $limit = 5): string 
{
    if ($limit <= 0) {
        return '<p>Parameter must be a positive integer.</p>';
    }
    ...
    $db = new Database();
    $sql = "SELECT a.id, a.title, a.publish_at, a.created_at, c.icon
            FROM articles a
            JOIN categories c ON a.category_id = c.id
            WHERE a.status = 'published'
              AND (a.publish_at IS NULL OR a.publish_at <= NOW())
    ...
    try {
        ...
        if (!$results) return '<p>No recent articles available.</p>';

        $html = '<ul class="top-articles recent">';
        foreach ($results as $row) {
            $iconPath = APP_URL . 'assets/icons/categories/' . htmlspecialchars($row['icon']);
            $titleShort = truncateText($row['title'], 35, '...');
            ...
            if ($diff < 3600) {
                ...
                $timeAgo = date('M d', $timestamp);
            }
            
            $html .= '<li class="grid-li">
                        <span><img src="' . $iconPath . '" class="li-icon" alt=""></span>
                        <span><a href="view_article.php?id=' . $row['id'] . '">' . htmlspecialchars($titleShort) . '</a></span>
                        <span class="span-1 recent-time">' . $timeAgo . '</span>
                      </li>';
        }
        $html .= '</ul>';
        return $html;
...
// Get all active categories for quick access grid
function getCategoriesGrid(): string 
{
    $db = new Database();
    $sql = "SELECT id, name, icon, description
            FROM categories
            WHERE is_active = 1
            ORDER BY name ASC
            LIMIT 10";
    ...
    try {
        ...
        foreach ($categories as $cat) {
            $iconPath = APP_URL . 'assets/icons/categories/' . htmlspecialchars($cat['icon']);
            $html .= '<div class="category-item" data-category-id="' . $cat['id'] . '" data-category-name="' . htmlspecialchars($cat['name']) . '" title="' . htmlspecialchars($cat['description'] ?? $cat['name']) . '">
                        <img src="' . $iconPath . '" alt="' . htmlspecialchars($cat['name']) . '">
                      </div>';
        }
        $html .= '</div>';
        return $html;
...
{
    $db = new Database();
    $sql = "SELECT t.id, t.name, COUNT(at.article_id) as usage_count
            FROM tags t
    ...
    try {
        ...
        if (!$tags) return '<span style="color: #888;">No tags available</span>';

        $html = '';
        foreach ($tags as $tag) {
            $html .= '<span class="tag-item" data-tag-id="' . $tag['id'] . '" title="' . htmlspecialchars($tag['name']) . ' (' . $tag['usage_count'] . ' articles)">' . htmlspecialchars($tag['name']) . '</span>';
        }
        return $html;

    } catch (PDOException $e) {
        return '<span>Error: ' . htmlspecialchars($e->getMessage()) . '</span>';
    }
}

/**
 * Get Reading Progress for the current user
 * Returns percentage of articles read (based on scroll >= 80% or sufficient reading time)
 */
function getReadingProgress(): array 
{
    if (!isset($_SESSION['user'])) {
        return [
            ...
            'articles_read' => 0,
            'percentage' => 0,
            'message' => 'Login to track your reading progress'
        ];
    }
    ...
    try {
        // Get total active published articles
        $totalSql = "SELECT COUNT(*) as total FROM articles WHERE status = 'published'";
        $stmt = $db->prepare($totalSql);
        $stmt->execute();
        $totalArticles = (int)$stmt->fetch()['total'];
        
        // Get articles read (scroll >= 80% OR reading_time >= 60 seconds)
        $readSql = "SELECT COUNT(DISTINCT article_id) as read_count
                    FROM article_reading_time 
                    WHERE user_id = :user_id 
                    AND (scroll_percentage >= 80 OR reading_time >= 60)";
        $stmt = $db->prepare($readSql);
        $stmt->execute(['user_id' => $userId]);
        $articlesRead = (int)$stmt->fetch()['read_count'];
        
        $percentage = $totalArticles > 0 ? round(($articlesRead / $totalArticles) * 100, 1) : 0;
...
// Get empty state HTML with suggestions
function getEmptyStateHTML(string $searchQuery = '', string $category = '', string $author = ''): string 
{
    $db = new Database();
    
    // Build context-aware message
    $message = '<div class="empty-state">';
    $message .= '<img src="' . APP_URL . 'assets/icons/icon-search.svg" class="empty-state-icon" alt="No results">';
    $message .= '<h3 class="empty-state-title">No Articles Found</h3>';
    
    if ($searchQuery) {
        $message .= '<p class="empty-state-text">We couldn\'t find any articles matching <strong>"' . htmlspecialchars($searchQuery) . '"</strong></p>';
    } elseif ($category) {
        $message .= '<p class="empty-state-text">No articles found in category <strong>"' . htmlspecialchars($category) . '"</strong></p>';
    } elseif ($author) {
        $message .= '<p class="empty-state-text">No articles found by author <strong>"' . htmlspecialchars($author) . '"</strong></p>';
    } else {
        $message .= '<p class="empty-state-text">No articles match your search criteria.</p>';
    }
    
    // Add suggestions
    $message .= '<div class="empty-state-suggestions">';
    $message .= '<p class="empty-state-subtitle">Try the following:</p>';
    $message .= '<ul class="empty-state-tips">';
    $message .= '<li>Check your spelling</li>';
    $message .= '<li>Use fewer or different keywords</li>';
    $message .= '<li>Try more general search terms</li>';
    $message .= '<li>Browse popular articles below</li>';
    $message .= '</ul>';
    $message .= '</div>';
    
    // Get popular articles as suggestions
    try {
        $sql = "SELECT a.id, a.title, c.icon, c.name as category
                FROM articles a
                JOIN categories c ON a.category_id = c.id
                WHERE a.status = 'published'
                  AND (a.publish_at IS NULL OR a.publish_at <= NOW())
        ...
        if ($popularArticles) {
            $message .= '<div class="empty-state-popular">';
            $message .= '<h4 class="empty-state-subtitle">Popular Articles:</h4>';
            $message .= '<ul class="empty-state-articles">';
            
            foreach ($popularArticles as $article) {
                $iconPath = APP_URL . 'assets/icons/categories/' . htmlspecialchars($article['icon']);
                $message .= '<li>';
                $message .= '<img src="' . $iconPath . '" class="article-icon" alt="">';
                $message .= '<a href="view_article.php?id=' . $article['id'] . '">' . htmlspecialchars($article['title']) . '</a>';
                $message .= '<span class="article-category">' . htmlspecialchars($article['category']) . '</span>';
                $message .= '</li>';
            }
        ...
        // Silently fail - empty state is still useful without suggestions
    }
...
function renderDraftsArticles(array $drafts, array $t): string {
    ...
    if (empty($drafts)) {
        $bodyContent = '<div style="color: #888; padding: 20px; text-align: center;">' . $t['lang_no_articles_in_draft'] . '</div>';
    } else {
        // Generate the list of draft articles
        $bodyContent = '<div class="draft-articles-list">';
        
        foreach ($drafts as $draft) {
            $bodyContent .= '
                <div class="draft-article-item">
                    <div class="draft-article-content">
                        <div class="draft-article-info">
                            <a href="view_article.php?id=' . $draft['article_id'] . '&version=' . $draft['version_number'] . '" class="draft-article-title">' 
                                . htmlspecialchars($draft['title']) . 
                            '</a>
                            <div class="draft-article-meta">
                                <span>'.$t['lang_art_created'].' ' . date('Y-m-d H:i', strtotime($draft['created_at'])) . '</span>
                            </div>
                        </div>
                        <div class="draft-article-actions">
                            <a href="#" onclick="openEditArticleModal('.$draft['article_id'].','.$draft['version_number'].');return false;" class="action-link" title="' . $t['lang_btn_edit']. '">
                                <img src="' . APP_URL . 'assets/icons/icon-edit.svg" class="op-icon">
                            </a>
                            <a href="#" onclick="submitForApproval(' . $draft['article_id'] . ', ' . ($draft['version_number'] ?? 1) . '); return false;" class="action-link" title="' . $t['lang_btn_send_approval'] . '">
                                <img src="' . APP_URL . 'assets/icons/icon-send-approval.svg" class="op-icon">
                            </a>
                        </div>
                    </div>
                </div>';
...
function renderPendingArticles(array $pendingArticles,array $t): string {
    ...
    if (empty($pendingArticles)) {
        $bodyContent = '<div style="color: #888; padding: 20px; text-align: center;">' . $t['lang_no_articles_in_pending'] . '</div>';
    } else {
        // Generate the list of pending articles
        $bodyContent = '<div class="pending-articles-list">';
                ...
                <div class="pending-article-item">
                    <div class="pending-article-content">
                        <div class="pending-article-info">
                            <a href="view_article.php?id='.$article['article_id'].'&version='.$article['version_number'].'" class="pending-article-title">' 
                                . htmlspecialchars($article['title']) . 
                            '</a>
                            <div class="pending-article-meta">
                                <span>Autor: ' . htmlspecialchars($article['username']) . '</span>
                                <span class="separator">|</span>
                                <span>creat la ' . date('Y-m-d H:i', strtotime($article['created_at'])) . '</span>
                            </div>
                        </div>
                        <div class="pending-article-actions">
                            <a href="#" onclick="approveArticle('.$article['article_id'].','.$article['version_number'].');return false;" class="action-link" title="' . $t['lang_article_approve']. '">
                                <img src="' . APP_URL . 'assets/icons/icon-edit.svg" class="op-icon">
                            </a>
                            <a href="#" onclick="rejectArticle('.$article['article_id'].','.$article['version_number'].');return false;" class="action-link" title="' . $t['lang_article_reject']. '">
                                <img src="' . APP_URL . 'assets/icons/icon-art-reject.svg" class="op-icon">
                            </a>
                        </div>
                    </div>
                </div>';
...
// ***********************************************************************************************
// render Top Viewed Articles with CardInfoBox
// @Param: $limit - number of articles to display
// @Param: $days - number of days to consider (for title)
// @Param: $t - translation array
// @Return: HTML string
// ***********************************************************************************************
function renderTopViewedArticles(int $limit = 5, int $days = 30, array $t = []): string {
    
    // Create a unique ID for this section
    $rootId = 'top-viewed-articles-root-' . uniqid();
    
    // Get the articles content
    $bodyContent = getTopViewedArticles($limit);
    
    // Build the title
    $title = $t['lang_db_top5_views'] . $days . $t['lang_db_art_days'];
    ...
    $html .= '<script>
        (function() {
            ...
            root.render(
                React.createElement(CardInfoBox, {
                    icon: "' . APP_URL . 'assets/icons/icon-top-art-views.svg",
                    title: ' . json_encode($title) . ',
                    body: React.createElement("div", {
                        dangerouslySetInnerHTML: { __html: bodyContent }
                    }),
...
// ***********************************************************************************************
// render Top Liked Articles with CardInfoBox
// @Param: $limit - number of articles to display
// @Param: $days - number of days to consider (for title)
// @Param: $t - translation array
// @Return: HTML string
// ***********************************************************************************************
function renderTopLikedArticles(int $limit = 5, int $days = 30, array $t = []): string {
    
    // Create a unique ID for this section
    $rootId = 'top-liked-articles-root-' . uniqid();
    
    // Get the articles content
    $bodyContent = getTopLikedArticles($limit);
    
    // Build the title
    $title = $t['lang_db_top5_likes'] . $days . $t['lang_db_art_days'];
    ...
    $html .= '<script>
        (function() {
            ...
            root.render(
                React.createElement(CardInfoBox, {
                    icon: "' . APP_URL . 'assets/icons/icon-top-art-likes.svg",
                    title: ' . json_encode($title) . ',
                    body: React.createElement("div", {
                        dangerouslySetInnerHTML: { __html: bodyContent }
                    }),
...
// ***********************************************************************************************
// render Top Commented Articles with CardInfoBox
// @Param: $days - number of days to consider
// @Param: $t - translation array
// @Return: HTML string
// ***********************************************************************************************
function renderTopCommentedArticles(int $days = 30, array $t = []): string {
    
    // Create a unique ID for this section
    $rootId = 'top-commented-articles-root-' . uniqid();
    
    // Get the articles content
    $bodyContent = getTop5CommentedArticles($days);
    
    // Build the title
    $title = $t['lang_db_top5_commented'] . $days . $t['lang_db_art_days'];
    ...
    $html .= '<script>
        (function() {
            ...
            root.render(
                React.createElement(CardInfoBox, {
                    icon: "' . APP_URL . 'assets/icons/icon-top-comments.svg",
                    title: ' . json_encode($title) . ',
                    body: React.createElement("div", {
                        dangerouslySetInnerHTML: { __html: bodyContent }
                    }),
...
function renderSuperAdminDashboard(): string {
    ...
    $artViewDays = 30;
    $html .= renderTopViewedArticles(5, $artViewDays, [
        'lang_db_top5_views' => lang('lang_db_top5_views'),
        'lang_db_art_days' => lang('lang_db_art_days')
    ]);
...
function renderDashboardBox($box, $data) {
    ...
    switch ($box) {
        ...
        case 'topViewed':
            ...
            return renderTopViewedArticles(5, 30, $lang_text);
        case 'topLiked':
            $lang_text = [
                'lang_db_top5_likes' => lang('lang_db_top5_likes'),
                'lang_db_art_days' => lang('lang_db_art_days')
            ];
            return renderTopLikedArticles(5, 30, $lang_text);
        case 'topCommented':
            $lang_text = [
                'lang_db_top5_commented' => lang('lang_db_top5_commented'),
                'lang_db_art_days' => lang('lang_db_art_days')
            ];
            return renderTopCommentedArticles(30, $lang_text);
        case 'articlesChart':
            $rootId = 'articles-chart-root-' . uniqid();
            $html = '<div id="' . $rootId . '"></div>';
            $html .= '<script>
                (function() {
                    const root = ReactDOM.createRoot(document.getElementById("' . $rootId . '"));
                    root.render(
                        React.createElement(CardInfoBox, {
                            icon: "' . APP_URL . 'assets/icons/icon-art-dashboard.svg",
                            title: "' . lang('lang_db_articles') . '",
                            body: React.createElement("div", {
                                style: { padding: "10px", width: "100%", height: "auto", position: "relative" },
                                dangerouslySetInnerHTML: { __html: \'<canvas id="articlesChart"></canvas>\' }
                            }), 
                            collapsible: true,
                            defaultOpen: false,
                            titleSize: "1.1em",
                    ...
                    setTimeout(function() {
                        ...
                        if (canvas && window.renderChart) {
                            // Re-trigger chart rendering if data is available
                            const event = new CustomEvent("canvasReady", { detail: { canvasId: "articlesChart" } });
            ...
            return $html;
        case 'commentsChart':
            $rootId = 'comments-chart-root-' . uniqid();
            $html = '<div id="' . $rootId . '"></div>';
            $html .= '<script>
                (function() {
                    const root = ReactDOM.createRoot(document.getElementById("' . $rootId . '"));
                    root.render(
                        React.createElement(CardInfoBox, {
                            icon: "' . APP_URL . 'assets/icons/icon-art-dashboard.svg",
                            title: "' . lang('lang_db_recent_comments') . '",
                            body: React.createElement("div", {
                                style: { padding: "10px", width: "100%", height: "auto", position: "relative" },
                                dangerouslySetInnerHTML: { __html: \'<canvas id="commentsChart"></canvas>\' }
                            }),
                            collapsible: true,
                            defaultOpen: false,
                            titleSize: "1.1em",
                    ...
                    setTimeout(function() {
                        ...
                        if (canvas && window.renderChart) {
                            // Re-trigger chart rendering if data is available
                            const event = new CustomEvent("canvasReady", { detail: { canvasId: "commentsChart" } });
                            document.dispatchEvent(event);
                        }
                    }, 100);
                })();
        ...
        case 'operations':
            ...
            $html .= '<script>
                (function() {
                    ...
                    root.render(
                        React.createElement(CardInfoBox, {
                            icon: "' . APP_URL . 'assets/icons/icon-operations.svg",
                            title: "Operațiuni",
                            body: React.createElement("div", {
                                dangerouslySetInnerHTML: { __html: \'<ul><li>18 create</li><li>7 editate</li><li>2 șterse</li></ul>\' }
                            }),
                            collapsible: true,
                            defaultOpen: false,
                            titleSize: "1.1em",
        ...
        case 'logs':
            ...
            $html .= '<script>
                (function() {
                    ...
                    root.render(
                        React.createElement(CardInfoBox, {
                            icon: "' . APP_URL . 'assets/icons/icon-logs.svg",
                            title: "Loguri",
                            body: React.createElement("div", {
                                dangerouslySetInnerHTML: { __html: \'<ul><li>104 azi</li><li>7 arhivate</li><li>3 șterse</li></ul>\' }
                            }),
                            collapsible: true,
                            defaultOpen: false,
                            titleSize: "1.1em",
        ...
        case 'exports':
            ...
            $html .= '<script>
                (function() {
                    ...
                    root.render(
                        React.createElement(CardInfoBox, {
                            icon: "' . APP_URL . 'assets/icons/icon-export.svg",
                            title: "Exporturi",
                            body: React.createElement("div", {
                                dangerouslySetInnerHTML: { __html: \'<ul><li>1 CSV azi</li><li>3 backup-uri</li><li>Ultimul: 2024-05-01</li></ul>\' }
                            }),
                            collapsible: true,
                            defaultOpen: false,
                            titleSize: "1.1em",
