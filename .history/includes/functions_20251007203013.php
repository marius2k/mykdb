...
function logArticleView($articleId) {
    ...
    */
    ...
    //error_log("Step4: Article view logged for article ID $articleId by user ID " . ($userId ?? 'guest') ." IP: $ip" . " User agent: $ua");
}
...
function generateNavBar2($uid) {
    ...
    if (hasPermission($uid,$ops)){
        ...
        if (in_array($userRole, ['moderator', 'editor', 'admin', 'superadmin'])) {
            $adminMenuItems .= '<a href="'.APP_URL.'public/admin/analytics.php" class="dropdown-item">
                    <img src="'.APP_URL.'assets/icons/icon-view.svg" class="submenu-icon"> Analytics
                </a>';
        }
    ...
    if ($hasAdminAccess) {
        $isAdminPageActive = in_array($currentPage, ['logs.php', 'users.php', 'categories.php', 'articles.php', 'analytics.php', 'tags.php', 'comments.php', 'acl_edit.php']);
