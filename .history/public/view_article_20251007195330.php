...
if ($articleId && $isOnline==1){
    try {
        ...
        //error_log("View incremented for article ID: $articleId");
    } catch (Exception $e) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Inițializează tracking-ul cu APP_URL
    window.readingTracker = new ReadingTimeTracker(<?= $article_id ?>, '<?= APP_URL ?>');
...
async function loadRelatedArticles(articleId) {
    try {
        const response = await fetch(`api/bkd_related_articles.php?id=${articleId}&limit=6`);
...
function displayRelatedArticles(articles) {
    ...
    let html = '<h3>📚 Articole Similare</h3><div class="related-articles-grid">';
    
    articles.forEach(article => {
        html += `
            <div class="related-article-card">
                <h4><a href="${article.url}">${article.title_truncated}</a></h4>
                ...
                ${article.excerpt_truncated ? `<p class="excerpt">${article.excerpt_truncated}</p>` : ''}
            </div>
        `;
    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const tracker = new ReadingTimeTracker({
        articleId: <?= $articleId ?>,
        apiUrl: '<?= APP_URL ?>public/api/bkd_article_reading_analytics.php',
        contentSelector: '.article-content, .trix-content',
        debug: false
    });
