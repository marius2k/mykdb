...
<div class="main" style="display: flex; flex-direction: row; gap: 20px;">
    ...
    <div class="defaultContent" style="width: 75%;">
        ...
        <div class="categories-tags-quick-access">
            <span class="section-label">Categories:</span>
            <?php echo getCategoriesGrid(); ?>
            <span class="section-label tags-label">Tags:</span>
            <div class="tags-grid">
                <?php echo getTopTags(); ?>
            </div>
        </div>
    ...
    <div class="rightSide" style="width: 25%; display: flex; flex-direction: column; gap: 20px; padding-top: 20px;">
        <!-- Search Box using CardInfoBox -->
        <div id="search-box-container"></div>
        
        <!-- Reading Progress Box using CardInfoBox -->
        <div id="reading-progress-container"></div>
        
        <!-- Recent Articles Box using CardInfoBox -->
        <div id="recent-box-container"></div>
    ...
    <div style="display: none;">
        ...
        <div id="reading-progress-content">
            <?php 
                $progress = getReadingProgress();
                if ($progress['message']) {
                    echo '<p style="text-align: center; color: #666;">' . htmlspecialchars($progress['message']) . '</p>';
                } else {
                    $remaining = $progress['total_articles'] - $progress['articles_read'];
                    echo '<div class="reading-progress-widget">
                            <div class="progress-circle-container">
                                <svg class="progress-ring" width="180" height="180" viewBox="0 0 180 180">
                                    <circle class="progress-ring-circle-bg" stroke="#e0e0e0" stroke-width="12" fill="transparent" r="78" cx="90" cy="90"/>
                                    <circle class="progress-ring-circle" stroke="var(--primary-color, #4e73df)" stroke-width="12" fill="transparent" r="78" cx="90" cy="90" 
                                        style="stroke-dasharray: 489.84; stroke-dashoffset: ' . (489.84 - (489.84 * $progress['percentage'] / 100)) . ';"/>
                                </svg>
                                <div class="progress-percentage">' . $progress['percentage'] . '%</div>
                            </div>
                            <div class="progress-cards">
                                <div class="progress-card">
                                    <span class="card-value">' . $progress['articles_read'] . '</span>
                                    <span class="card-label">Completed</span>
                                </div>
                                <div class="progress-card">
                                    <span class="card-value">' . $remaining . '</span>
                                    <span class="card-label">Remaining</span>
                                </div>
                                <div class="progress-card total-card">
                                    <span class="card-value">' . $progress['total_articles'] . '</span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const progressContent = document.getElementById('reading-progress-content');
    const progressRoot = ReactDOM.createRoot(progressContainer);
    
    const progressIcon = React.createElement('img', {
        src: '<?= APP_URL ?>assets/icons/icon-analytics.svg',
        width: '25',
        height: 'auto'
    });
    ...
    progressRoot.render(
        React.createElement(CardInfoBox, {
            icon: progressIcon,
            title: '<?= lang('lang_reading_progress') ?>',
            body: progressContent.innerHTML,
            defaultOpen: true,
            collapsible: true,
            height: "90px",
    ...
    // Recent Articles Box
    const recentContainer = document.getElementById('recent-box-container');
    const recentContent = document.getElementById('recent-content');
    const recentRoot = ReactDOM.createRoot(recentContainer);
    
    const recentIcon = React.createElement('img', {
        src: '<?= APP_URL ?>assets/icons/icon-calendar.svg',
        width: '25',
        height: 'auto'
    });
    ...
    recentRoot.render(
        React.createElement(CardInfoBox, {
            icon: recentIcon,
            title: '<?= lang('lang_article_recent') ?>',
            body: recentContent.innerHTML,
            defaultOpen: true,
            collapsible: true,
            height: "90px",
            style: { marginBottom: '4px' }
...
function triggerAdvancedSearch() {
  ...
  fetch('api/bkd_search_articles.php?' + params.toString())
    ...
    .then(text => {
        console.log('Search query:', query, 'Author:', author, 'Category:', category);
...
// Category Quick Access - Click handler
document.addEventListener('DOMContentLoaded', function() {
    ...
    categoryItems.forEach(item => {
        item.addEventListener('click', function() {
            ...
            // Remove active from all categories and tags
            ...
            if (!isActive) {
                ...
                // Set the category in search box
                const searchCategorySelect = document.getElementById('searchCategory');
                if (searchCategorySelect) {
                    $(searchCategorySelect).val(categoryName).trigger('change');
                }
                
                // Trigger search
                triggerAdvancedSearch();
            } else {
                // Deactivate - show all articles
                const searchCategorySelect = document.getElementById('searchCategory');
                if (searchCategorySelect) {
                    $(searchCategorySelect).val(null).trigger('change');
                }
    ...
    tagItems.forEach(item => {
        item.addEventListener('click', function() {
            ...
            // Remove active from all tags and categories
...
function searchByTag(tagName) {
    ...
    fetch('api/bkd_search_articles.php?' + params.toString())
        ...
        .then(data => {
            console.log('Tag search results:', data);
            defaultContent.style.display = 'none';
            
            if (data.length === 0) {
                searchResults.innerHTML = '<p class="no-results">No articles found with tag "' + tagName + '".</p>';
            } else {
                searchResults.innerHTML = data.map(article => `
                    <div class="article-card">
                      <h4><img src="<?=APP_URL?>/assets/icons/categories/${article.icon}" width="40" height="auto">&nbsp;&nbsp;&nbsp;&nbsp;${article.title}</h4>
                      <p class="article-meta" style="font-size: 0.9rem;"><em><?=lang('lang_art_author')?>: ${article.username} | <?=lang('lang_art_category')?>: ${article.category} | <?=lang('lang_art_publish_at')?>: ${new Date(article.publish_at).toLocaleDateString(window.TZ)}</em></p>
                      <p>${article.content}</p>
                      <p><a href="view_article.php?id=${article.id}&version=${article.version}">Read more...</a></p>
                    </div>
        ...
        .catch(err => {
            ...
            searchResults.innerHTML = '<p>Error searching by tag. Please try again.</p>';
        });
}
