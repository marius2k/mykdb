    ...
    articles.forEach(article => {
        html += `
            <div class="related-article-card">
                <h4><a href="view_article.php?id=${article.id}&version=1&isonline=1">${article.title}</a></h4>
                ...
                ${article.tags && article.tags.length > 0 ? `<p class="excerpt">🏷️ ${article.tags.join(', ')}</p>` : ''}
            </div>
        `;
    });
