...
function buildPublishAtHtml(articleId, status, publishAt, version = null, onlineVersion = null, isOnline = null) {
    // Logica simplificată bazată doar pe status
    if (status === 'published') {
        // Versiunea este publicată - afișează doar data (read-only)
        return publishAt ? new Date(publishAt).toLocaleDateString('ro-RO') : '-';
        ...
        // Alte statusuri (draft, pending, rejected) - nu se afișează nimic
        return '-';
    }
}
