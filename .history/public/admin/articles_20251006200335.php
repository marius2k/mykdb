        ...
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) { 
                    ...
                    if (isOnlineVersion) {
                        return `<span class="online-label">ONLINE</span><span${titleAttr}>${truncatedTitle}</span>`;
                    } else {
                        return `<span class="offline-label">OFFLINE</span><span${titleAttr}>${truncatedTitle}</span>`;
                    }
                } 
            },
            {
    ...
    $('#articlesTable tbody').on('change', '.version-select', function() {
        ...
        fetch(url)
            ...
            .then(function(data) {
                ...
                // Actualizează titlul (coloana 2) cu label-ul ONLINE/OFFLINE
                ...
                if (isOnlineVersion) {
                    titleHtml = `<span class="online-label">ONLINE</span>&nbsp;&nbsp;&nbsp;<span${titleAttr}>${truncatedTitle}</span>`;
                } else {
                    titleHtml = `<span class="offline-label">OFFLINE</span>&nbsp;&nbsp;&nbsp;<span${titleAttr}>${truncatedTitle}</span>`;
                }
...
function updateOnlineLabels() {
    if (typeof table !== 'undefined' && table) {
        table.rows().every(function() {
            ...
            if (row && node) {
                const titleCell = node.querySelector('td:nth-child(2)'); // Coloana titlului
                if (titleCell) {
                    const isOnlineVersion = row.is_online == 1;
                    
                    // Elimină label-urile existente dacă există
                    const existingOnlineLabel = titleCell.querySelector('.online-label');
                    const existingOfflineLabel = titleCell.querySelector('.offline-label');
                    
                    if (existingOnlineLabel) {
                        existingOnlineLabel.remove();
                        // Elimină și spațiile care urmează
                        const nextSibling = existingOnlineLabel.nextSibling;
                        if (nextSibling && nextSibling.nodeType === Node.TEXT_NODE) {
                            nextSibling.remove();
                        }
                    }
                    
                    if (existingOfflineLabel) {
                        existingOfflineLabel.remove();
                        // Elimină și spațiile care urmează
                        const nextSibling = existingOfflineLabel.nextSibling;
                        if (nextSibling && nextSibling.nodeType === Node.TEXT_NODE) {
                            nextSibling.remove();
                        }
                    }
                    
                    // Adaugă label-ul corespunzător la început
                    ...
                    titleCell.insertBefore(document.createTextNode('\u00A0\u00A0\u00A0'), label.nextSibling);
                }
            }
        });
    }
...
function generatePublishAtForArticle(row) {
    ...
    return buildPublishAtHtml(row.article_id || 0, versionStatus, row.publish_at || '', selectedVersion, onlineVersion);
}
...
function generatePublishAtForArticleWithVersionData(articleId, versionData, rowData) {
    ...
    return buildPublishAtHtml(articleId, versionData.status, rowData.publish_at || '', versionData.version_number, onlineVersion);
}
...
function buildPublishAtHtml(articleId, status, publishAt, version = null, onlineVersion = null) {
    const data = publishAt || '';
    const isSelectedVersionOnline = version && onlineVersion && parseInt(version) === parseInt(onlineVersion);
    
    // Cazul 1: Versiuni offline cu status "approved" (gata pentru publicare)
    if ((status || '') === 'approved' && !isSelectedVersionOnline) {
        if (data) {
            // Dacă există dată, afișează-o (cu culoare portocalie dacă este în viitor)
            ...
            // Dacă nu există dată, permite setarea unei date (doar pentru roluri autorizate)
    ...
    // Cazul 2: Versiuni online cu status "disabled" (care pot fi restaurate)
    else if ((status || '') === 'disabled' && isSelectedVersionOnline) {
        if (data) {
            // Dacă există dată, afișează-o (cu culoare portocalie dacă este în viitor)
            const pubDate = new Date(data.replace(' ', 'T'));
            const now = new Date();
    ...
    // Pentru toate celelalte cazuri, afișează doar datele sau "-"
    else {
        if (data) {
            return `<span>${data}</span>`;
        } else {
...
function renderArticlesTable(articles) {
    ...
    articles.forEach(a => {
        ...
        const publishAtHtml = buildPublishAtHtml(a.id, a.status, a.publish_at || '', crtVersion, onlineVersion);
