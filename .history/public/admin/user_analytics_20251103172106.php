...
async function loadSearchBehaviorData() {
    try {
        ...
        const response = await fetch(`${APP_URL}public/api/bkd_user_analytics.php?action=search_behavior&start=${startDate}&end=${endDate}&role=${roleFilter}`);
        ...
        const data = await response.json();
        console.log('Search behavior data:', data);
                ...
                { 
                    ...
                    render: function(data, type, row) {
                        if (data && row.article_id) {
                            return `<a href="${APP_URL}public/view_article.php?id=${row.article_id}" target="_blank">${data}</a>`;
                        }
                        return '-';
                    }
                },
        ...
        showError('Error loading search behavior data');
    }
}
