...
function displayWeeklyStats(stats) {
    ...
    container.innerHTML = `
        <div class="stat-card views">
            ...
            <div class="stat-label">👁️ ${TRANSLATIONS.public_views_this_week || 'Public Views This Week'}</div>
            <div class="stat-sublabel" style="font-size: 0.8em; color: #95a5a6; margin-top: 2px;">
                Admin: ${formatNumber(stats.weekly_admin_views || 0)}
            </div>
        </div>
        <div class="stat-card likes">
            <div class="stat-value">${formatNumber(stats.weekly_likes || 0)}</div>
            <div class="stat-label">❤️ ${TRANSLATIONS.likes_this_week}</div>
        </div>
        <div class="stat-card reading">
            <div class="stat-value">${formatTime(stats.avg_weekly_public_reading_time || 0)}</div>
            <div class="stat-label">📚 ${TRANSLATIONS.avg_public_reading_time || 'Avg Public Reading Time'}</div>
            <div class="stat-sublabel" style="font-size: 0.8em; color: #95a5a6; margin-top: 2px;">
                Admin: ${formatTime(stats.avg_weekly_admin_reading_time || 0)}
            </div>
        </div>
        <div class="stat-card engagement">
            <div class="stat-value">${formatNumber(stats.weekly_public_reading_sessions || 0)}</div>
            <div class="stat-label">🔥 ${TRANSLATIONS.public_reading_sessions || 'Public Reading Sessions'}</div>
            <div class="stat-sublabel" style="font-size: 0.8em; color: #95a5a6; margin-top: 2px;">
                Admin: ${formatNumber(stats.weekly_admin_reading_sessions || 0)}
            </div>
        </div>
    `;
}
...
function displayPerformanceChart(articles) {
    ...
    performanceChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: TRANSLATIONS.public_views || 'Public Views',
                ...
                {
                    label: TRANSLATIONS.admin_views || 'Admin Views',
                    data: adminViewsData,
                ...
                {
                    label: TRANSLATIONS.public_reading_time_seconds || 'Public Reading Time (sec)',
    ...
    container.innerHTML = `
        ...
        <div class="stats-grid">
            <div class="stat-card views">
                ...
                <div class="stat-label">👁️ ${TRANSLATIONS.total_views}</div>
            </div>
            <div class="stat-card likes">
                <div class="stat-value">${formatNumber(stats.total_likes || 0)}</div>
                <div class="stat-label">❤️ ${TRANSLATIONS.total_likes}</div>
            </div>
            <div class="stat-card reading">
