...
const USER_ANALYTICS_TRANSLATIONS = {
    ...
    no_data: '<?= lang('lang_analytics_no_data') ?? 'No data available' ?>',
...
async function checkTablesExist() {
    try {
        ...
        return data.success && data.tables_exist;
    } catch (error) {
...
async function loadContentInteractionData() {
    try {
        ...
        if (!tablesExistResult) {
            ...
            // Display empty data to prevent errors
        ...
        console.log('Fetching content interaction data...');
        document.getElementById('content-loading-indicator').style.display = 'block'; // Show loading indicator if it exists
        ...
        if (data.success) {
            // Verify that we have all the expected data properties
            ...
            // Display empty data when there's an error
            displayContentInteractionOverview(null);
            displayUserActionsChart(null);
            displayArticleInteractionTable(null);
        }
...
function displayContentInteractionOverview(stats) {
    ...
    if (!stats || typeof stats !== 'object') {
        console.log('No content interaction stats available');
        // Set default values
        document.getElementById('total-bookmarks').textContent = '0';
        document.getElementById('avg-rating').textContent = '0.0/5.0';
        document.getElementById('useful-ratio').textContent = '0%';
...
function displayArticleInteractionTable(articles) {
    try {
        // Always destroy existing DataTable if it exists
        if ($.fn.DataTable.isDataTable('#articleInteractionTable')) {
            $('#articleInteractionTable').DataTable().destroy();
        }
        
        // Get tbody element
        const table = document.getElementById('articleInteractionTable');
        if (!table) {
            console.error('Could not find articleInteractionTable element');
            return;
        }
        ...
        if (!articles || !Array.isArray(articles) || articles.length === 0) {
            console.log('No article interaction data available');
            
            // Create a single row with a colspan matching the number of headers
            const headerCells = table.querySelectorAll('thead th').length;
            const colspan = headerCells || 8; // Default to 8 if headers can't be counted
            
            const row = tbody.insertRow();
            row.innerHTML = `
                <td colspan="${colspan}" class="text-center">${USER_ANALYTICS_TRANSLATIONS.no_data || 'No data available'}</td>
            `;
            
            // Initialize DataTable with minimal options for empty table
            try {
                ...
                console.error('Error initializing empty DataTable:', error);
            }
            return;
        }
        ...
        try {
            articles.forEach((article, index) => {
                if (!article) {
                    console.warn(`Article at index ${index} is undefined, skipping`);
                    return;
                }
                ...
                // Create a new row with exactly the same number of cells as the header
                const row = tbody.insertRow();
                row.innerHTML = `
                    <td>${articleId}</td>
                    <td><a href="${APP_URL}public/view_article.php?id=${articleId}" target="_blank">${truncateText(title, 50)}</a></td>
            ...
            // If there's an error, clear the table and show an error message
            tbody.innerHTML = '';
            
            // Get the correct column count
            const headerCells = table.querySelectorAll('thead th').length;
            const colspan = headerCells || 8; // Default to 8 if headers can't be counted
            
            const row = tbody.insertRow();
            row.innerHTML = `
                <td colspan="${colspan}" class="text-center">Error displaying data. Please try again.</td>
            `;
            
            // Try to initialize a simple DataTable without features
            try {
                $('#articleInteractionTable').DataTable({
                    searching: false,
                    ordering: false,
                ...
                console.error('Could not initialize fallback DataTable:', e);
            }
        }
    } catch (error) {
...
// Helper function to check if analytics tables exist
async function checkTablesExist() {
    try {
        const response = await fetch(`<?= APP_URL ?>public/api/bkd_user_analytics.php?action=check_tables`);
        if (!response.ok) {
            console.error('Server returned error when checking tables:', response.status, response.statusText);
            return false;
        }
