...
<div class="breadcrumb-filter-section" style="display: flex; justify-content: space-between; align-items: center; width: 100vw; padding: 6px 20px; margin-top: 0; margin-bottom: 0; margin-left: calc(-50vw + 50%);">
    ...
    <div style="display: flex; gap: 15px; align-items: center; margin-right: 20px;">
        <div style="display: flex; align-items: center; gap: 8px;">
            <label for="role-filter" style="color: #666; font-size: 14px; margin: 0;"><?= lang('lang_analytics_role') ?>:</label>
            <select id="role-filter" class="form-control" style="width: 140px; height: 32px; font-size: 14px; padding: 4px 8px; margin: 0; border: 1px solid #ccc; border-radius: 4px;">
        ...
        <div style="display: flex; align-items: center; gap: 8px;">
            <label for="time-period" style="color: #666; font-size: 14px; margin: 0;"><?= lang('lang_analytics_time_period') ?>:</label>
            <select id="time-period" class="form-control" style="width: 140px; height: 32px; font-size: 14px; padding: 4px 8px; margin: 0; border: 1px solid #ccc; border-radius: 4px;">
    ...
    <div class="tab-content active" id="user-engagement-tab">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card" style="border-left-color: #f1c40f;">
                ...
                <div class="stat-label"><?= lang('lang_analytics_retention_rate') ?></div>
            </div>
        ...
        <!-- Most Engaged Users Table wrapped in CardInfoBox -->
        <div id="most-engaged-users-root"></div>
        
        <!-- Engagement by Role Chart wrapped in CardInfoBox -->
        <div id="engagement-by-role-root"></div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // Render CardInfoBox components for User Engagement sections
    renderEngagementOverTimeCard();
...
function displayUserEngagementOverview(retention) {
    ...
    const returningUsers = document.getElementById('returning-users');
    const avgInteractions = document.getElementById('avg-interactions');
    const powerUsers = document.getElementById('power-users');
    
    if (totalActiveUsers) totalActiveUsers.textContent = formatNumber(retention.total_users || 0);
    if (returningUsers) returningUsers.textContent = formatNumber(retention.returning_users || 0);
    if (avgInteractions) avgInteractions.textContent = calculateRetentionRate(retention.returning_users, retention.total_users) + '%';
    if (powerUsers) powerUsers.textContent = formatNumber(retention.power_users || 0);
}
...
// Render functions for CardInfoBox components in User Engagement tab
function renderEngagementOverTimeCard() {
    const root = ReactDOM.createRoot(document.getElementById('engagement-over-time-root'));
    ...
    root.render(
        React.createElement(CardInfoBox, {
            icon: '<?= APP_URL ?>assets/icons/icon-user-eng-over-time.svg',
            title: React.createElement('span', { id: 'title-engagement-over-time' }, '<?= lang('lang_analytics_engagement_over_time') ?>'),
            body: bodyContent,
            defaultOpen: false
        })
    );
...
function renderMostEngagedUsersCard() {
    const root = ReactDOM.createRoot(document.getElementById('most-engaged-users-root'));
        ...
        React.createElement('table', {
            ...
            React.createElement('thead', null,
                React.createElement('tr', null,
                    ...
                    React.createElement('th', null, '<?= lang('lang_analytics_email') ?>'),
                    React.createElement('th', null, '<?= lang('lang_analytics_role') ?>'),
                    React.createElement('th', null, '<?= lang('lang_analytics_articles_viewed') ?>'),
                    React.createElement('th', null, '<?= lang('lang_analytics_articles_bookmarked') ?>'),
                    React.createElement('th', null, '<?= lang('lang_analytics_articles_commented') ?>'),
                    React.createElement('th', null, '<?= lang('lang_analytics_articles_rated') ?>'),
    ...
    root.render(
        React.createElement(CardInfoBox, {
            icon: '<?= APP_URL ?>assets/icons/icon-user-eng-most.svg',
            title: React.createElement('span', { id: 'title-most-engaged-users' }, '<?= lang('lang_analytics_most_engaged_users') ?>'),
            body: bodyContent,
            defaultOpen: false
        })
    );
...
function renderEngagementByRoleCard() {
    const root = ReactDOM.createRoot(document.getElementById('engagement-by-role-root'));
...
function displayEngagementChart(engagementData) {
    ...
    setTimeout(() => {
        const canvasElement = document.getElementById('engagementChart');
        if (!canvasElement) {
            console.error('engagementChart canvas not found');
            return;
        }
