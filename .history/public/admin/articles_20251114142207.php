    ...
    const table = $('#articlesTable').DataTable({
        ...
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) { 
                    const title = data || '';
                    // Truncează titlul la 50 de caractere
                    const truncatedTitle = title.length > 50 ? title.substring(0, 50) + '...' : title;
                    
                    // Adaugă title attribute pentru titlul complet la hover
                    const titleAttr = title.length > 50 ? ` title="${title}"` : '';
            ...
            { data: 'status', className: 'dt-center', targets: [5], render: function(data, type, row) { return `<span class='version-status'>${data || ''}</span>`; } },
            { 
                data: 'publish_at',
                render: function(data, type, row) {
    ...
    $('#articlesTable tbody').on('change', '.version-select', function() {
        ...
        fetch(url)
            ...
            .then(function(data) {
                ...
                // Update DataTable's internal row data with the new version information
                rowData.current_version = parseInt(version);
                rowData.status = data.status;
                rowData.is_online = data.is_online;
                rowData.title = data.title;
                rowData.username = data.username || rowData.username;
                rowData.category = data.category || rowData.category;
                rowData.updated_at = data.updated_at;
                rowData.version_status = data.status;
                // Update article_status from API response (articles_status for online versions)
                if (data.articles_status) {
                    rowData.article_status = data.articles_status;
                } else if (data.is_online == 1) {
                    // If online but no articles_status provided, keep the existing one
                    rowData.article_status = rowData.article_status;
                } else {
                    // For offline versions, article_status is not relevant
                    rowData.article_status = null;
                }
                ...
                titleHtml = `<span${titleAttr}><a href="#" onClick="viewArticleWithVersion(${articleId}, ${version});return false;">${truncatedTitle}</a></span>`;
                tr.find('td:nth-child(2)').html(titleHtml);
                
                // Actualizează coloana Author (coloana 4) - get username from API or keep current
                if (data.username) {
                    tr.find('td:nth-child(4)').text(data.username);
                }
                
                // Actualizează coloana Category (coloana 5) - get category from API or keep current  
                if (data.category) {
                    tr.find('td:nth-child(5)').text(data.category);
                }
                ...
                const actionsHtml = generateActionsForArticleWithVersionData(articleId, data, rowData);
...
function generateActionsForArticle(row) {
    ...
    // Get the actual online version number from versions array
    const onlineVersion = getOnlineVersionNumber(row);
    
    // DEBUG - Enable for all articles temporarily
    /*
    console.log(`🔍 DEBUG Article ${row.article_id} actions at initial load:`, {
        ...
        rowArticleStatus: row.article_status
    });
    */
...
function generateActionsForArticleWithVersionData(articleId, versionData, rowData) {
    // Get the actual online version number from versions array
    const onlineVersion = getOnlineVersionNumber(rowData);
    return buildActionsHtml(articleId, versionData.status, rowData.publish_at || '', versionData.version_number, rowData.user_id || 0, onlineVersion, rowData.article_status || null);
}
...
function buildActionsHtml(articleId, status, publishAt, version = null, authorId = null, onlineVersion = null, articleStatus = null) {
    ...
    if (isSelectedVersionOnline && articleStatus) {
        // Pentru versiuni online, folosește statusul din articles (published/disabled)
        displayStatus = articleStatus;
    }
    
    // DEBUG: Log pentru unapprove action visibility
    console.log(`🔍 buildActionsHtml for article ${articleId}:`, {
        version: version,
        onlineVersion: onlineVersion,
    ...
    // Definește acțiunile disponibile cu configurare
    const actionConfig = {
        view: {
            icon: 'icon-view.svg',
        ...
        unapprove: {
            icon: 'icon-unapprove.svg',
            title: 'Return to Pending',
            onclick: `unapproveArticle(${articleId}, ${version || `getSelectedVersion(${articleId})`});return false;`
        },
        approve: {
    ...
    // Itera prin configurare și generează butoane
    for (const [actionName, config] of Object.entries(actionConfig)) {
        const isEnabled = isActionEnabled(actionName, userRole, displayStatus, authorId, currentUserId, onlineVersion, version);
        
        // Logică specială pentru disable/restore - afișează doar una
        if (actionName === 'disable' && isSelectedVersionOnline && displayStatus === 'disabled') {
            continue; // Skip disable pentru articole disabled
        }
        if (actionName === 'restore' && (!isSelectedVersionOnline || displayStatus !== 'disabled')) {
            continue; // Skip restore pentru articole care nu sunt disabled online
        }
        
        if (isEnabled) {
            const cssClass = 'op-icon';
            html += `<a href="#" onclick="${config.onclick}"><img src="<?=APP_URL?>assets/icons/${config.icon}" class="${cssClass}" title="${config.title}" style="width: 32px;"></a> `;
        }
    }
        ...
        case 'editor':
            switch (actionName) {
                ...
                case 'history':
                    ...
                    return true;
                case 'edit':
                    // Editor: draft, pending + versiuni online disabled (NOT approved)
                    if (isSelectedVersionOnline) {
                        return status === 'disabled';
                    }
                    return status === 'draft' || status === 'pending';
                case 'unapprove':
                    // Editor: poate returna approved la pending (doar versiuni NON-online)
                    const unapproveEnabled = status === 'approved' && !isSelectedVersionOnline;
                    console.log(`🔍 Editor unapprove check:`, {
                        status: status,
                        isSelectedVersionOnline: isSelectedVersionOnline,
                        enabled: unapproveEnabled
                    });
                    return unapproveEnabled;
                case 'approve':
        ...
        case 'moderator':
            switch (actionName) {
                ...
                case 'history':
                    ...
                    return true;
                case 'edit':
                    // Moderator: draft, pending + versiuni online disabled (NOT approved)
                    if (isSelectedVersionOnline) {
                        return status === 'disabled';
                    }
                    return status === 'draft' || status === 'pending';
                case 'unapprove':
                    // Moderator: poate returna approved la pending (doar versiuni NON-online)
                    return status === 'approved' && !isSelectedVersionOnline;
                case 'approve':
        ...
        case 'admin':
            switch (actionName) {
                ...
                case 'history':
                    ...
                    return true;
                case 'edit':
                    // Admin: toate versiunile + versiuni online doar dacă sunt disabled (NOT approved)
                    if (isSelectedVersionOnline) {
                        return status === 'disabled';
                    }
                    return status === 'draft' || status === 'pending';
                case 'unapprove':
                    // Admin: poate returna approved la pending (doar versiuni NON-online)
                    return status === 'approved' && !isSelectedVersionOnline;
                case 'approve':
        ...
        case 'superadmin':
            switch (actionName) {
                ...
                case 'history':
                    ...
                    return true;
                case 'edit':
                    // Superadmin: toate versiunile + versiuni online doar dacă sunt disabled (NOT approved)
                    if (isSelectedVersionOnline) {
                        return status === 'disabled';
                    }
                    return status === 'draft' || status === 'pending';
                case 'unapprove':
                    // Superadmin: poate returna approved la pending (doar versiuni NON-online)
                    return status === 'approved' && !isSelectedVersionOnline;
                case 'approve':
...
// Helper function to get the online version number from row data
function getOnlineVersionNumber(row) {
    if (!row.versions || !row.versions.length) {
        // If no versions array, check if current row is online
        return row.is_online == 1 ? row.current_version : null;
    }
    
    // Find the version that is online
    const onlineVersion = row.versions.find(v => v.is_online == 1);
    return onlineVersion ? onlineVersion.version_number : null;
}
...
function viewArticleWithVersion(articleId, onlineVersion = null) {
    ...
    const url = `../view_article.php?id=${articleId}&version=${selectedVersion}&isonline=${isOnlineVersion}&admin_source=admin_preview`;
    ...
    articles.forEach(a => {
        ...
        html += `<tr style="font-size: 0.8em;">
            ...
            <td style="width:220px; text-align:center;">
                <a href="../view_article.php?id=${a.id}&version=${crtVersion}&admin_source=admin_preview"><img src="<?=APP_URL?>assets/icons/icon-view.svg" class="op-icon" title="<?= lang('lang_btn_view') ?>"></a>
...
// Funcție pentru acțiunea "Unapprove" - returnează un articol aprobat la starea pending
function unapproveArticle(articleId, version = null) {
    const selectedVersion = version || getSelectedVersion(articleId);
    
    // Solicită motivul pentru unapprove
    const reason = prompt('Motivul pentru returnarea acestui articol la starea "Pending":\n(Acest mesaj va fi vizibil autorului)', '');
    
    // Dacă utilizatorul anulează sau nu introduce un motiv, abandonează
    if (reason === null) {
        return; // Utilizatorul a apăsat Cancel
    }
    
    if (reason.trim() === '') {
        alert('Trebuie să introduci un motiv pentru returnarea articolului.');
        return;
    }
    
    const body = `action=unapprove&article_id=${encodeURIComponent(articleId)}&version=${encodeURIComponent(selectedVersion)}&reason=${encodeURIComponent(reason)}&csrf_token=${encodeURIComponent(window.CSRF_TOKEN)}`;
    
    fetch('../api/bkd_article_unapprove.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: body
    })
    ...
    .then(data => {
        if (data.success) {
            alert('Articolul a fost returnat la starea "Pending" cu succes!');
            reloadArticlesTable();
        } else {
            alert(data.error || 'Eroare la returnarea articolului!');
        }
    })
    .catch(error => {
...
function viewVersion(versionNumber, isOnline = 0) {
    ...
    const url = `../view_article.php?id=${articleId}&version=${versionNumber}&isonline=${isOnlineVersion ? 1 : 0}&admin_source=admin_preview`;
    window.location.href = url;
}
