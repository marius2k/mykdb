...
function buildPublishAtHtml(articleId, status, publishAt, version = null, onlineVersion = null, isOnline = null) {
    // DEBUG: Verifică ce status primește funcția
    console.log(`🔍 DEBUG buildPublishAtHtml - Article ${articleId}, Status: "${status}", PublishAt: "${publishAt}"`);
    
    // Logica corectă pentru Publication Date
    if (status === 'published') {
        // Versiunea este publicată - afișează doar data (read-only)
        console.log(`📅 Article ${articleId} - Status PUBLISHED: returning read-only text`);
        return publishAt ? publishAt : '-';
    } else if (status === 'approved') {
        // Versiunea este aprobată - poate fi publicată, deci data trebuie să fie editabilă
        console.log(`📅 Article ${articleId} - Status APPROVED: returning editable input`);
        ...
        // Versiunea este disabled - afișează doar data (read-only)
        return publishAt ? publishAt : '-';
    } else {
        // Draft, pending - nu au fost niciodată publicate, deci nu afișez nimic
        return '-';
    }
}
