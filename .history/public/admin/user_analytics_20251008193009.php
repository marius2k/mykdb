...
function displayArticleInteractionTable(articles) {
    try {
        ...
        if (!tbody) {
            console.error('Could not find articleInteractionTable tbody element');
            return;
        }
        ...
        if (!articles || !Array.isArray(articles) || articles.length === 0) {
            ...
            // Add a single row to show no data message
            try {
                const row = tbody.insertRow();
                const colspan = 8; // Adjust based on your table's column count
                row.innerHTML = `
                    <td colspan="${colspan}" class="text-center">${USER_ANALYTICS_TRANSLATIONS.no_data || 'No data available'}</td>
                `;
                
                // Initialize empty DataTable with minimal options
                ...
                console.error('Error displaying empty table:', error);
            }
            return;
        }
