        ...
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) { 
                    ...
                    const isOnlineVersion = selectedVersion && row.current_version && 
                                          parseInt(selectedVersion) === parseInt(row.current_version);
                    
                    if (isOnlineVersion) {
                        return `${title}&nbsp;&nbsp;&nbsp;<span class="online-label">ONLINE</span>`;
                    } else {
                        return title;
                    }
                } 
    ...
    $('#articlesTable tbody').on('change', '.version-select', function() {
        ...
        fetch(url)
            ...
            .then(function(data) {
                ...
                // Actualizează titlul (coloana 2) cu label-ul ONLINE dacă este cazul
                const rowData = tr.closest('table').DataTable().row(tr).data();
                const isOnlineVersion = version && rowData.current_version && 
                                      parseInt(version) === parseInt(rowData.current_version);
                let titleHtml = data.title || '';
                if (isOnlineVersion) {
                    titleHtml += `&nbsp;&nbsp;&nbsp;<span class="online-label">ONLINE</span>`;
                }
...
function updateOnlineLabels() {
    if (typeof table !== 'undefined' && table) {
        table.rows().every(function() {
            ...
            if (row && node) {
                const titleCell = node.querySelector('td:first-child');
                if (titleCell) {
                    const selectedVersion = getSelectedVersion(row.article_id || 0);
                    const isOnlineVersion = selectedVersion && row.current_version && 
                                          parseInt(selectedVersion) === parseInt(row.current_version);
                    ...
                    // Adaugă label-ul dacă este versiunea online
                    if (isOnlineVersion) {
