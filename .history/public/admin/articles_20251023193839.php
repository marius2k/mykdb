    ...
    $('#articlesTable tbody').on('change', '.version-select', function() {
        ...
        fetch(url)
            ...
            .then(function(data) {
                ...
                // Actualizează coloana Author (coloana 4)
                if (data.username) {
                ...
                // Actualizează datele interne ale DataTable pentru această linie
                // astfel încât operațiile ulterioare să folosească versiunea corectă
                rowData.current_version = data.version_number;
                rowData.status = data.status;
                rowData.title = data.title;
                rowData.is_online = data.is_online;
                rowData.publish_at = data.publish_at;
                rowData.updated_at = data.updated_at;
                rowData.user_id = data.author_id || data.user_id;
                rowData.author_id = data.author_id || data.user_id;
                rowData.username = data.username;
                
                // Invalidează și actualizează rândul în DataTable
...
function generateActionsForArticleWithVersionData(articleId, versionData, rowData) {
    // Folosește author_id din versionData (versiunea selectată) nu user_id din rowData (versiunea inițială)
    const authorId = versionData.author_id || versionData.user_id || rowData.user_id || 0;
    // onlineVersion ar trebui să fie versiunea care este online, nu current_version
    const onlineVersion = rowData.online_version || rowData.current_version || null;
    return buildActionsHtml(articleId, versionData.status, versionData.publish_at || '', versionData.version_number, authorId, onlineVersion);
}
