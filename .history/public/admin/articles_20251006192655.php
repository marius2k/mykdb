...
function buildPublishAtHtml(articleId, status, publishAt) {
    ...
    // Publication Date apare doar când status este 'approved'
    ...
    } else if ((status || '') === 'approved' && (window.USER_ROLE === 'moderator' || window.USER_ROLE === 'admin' || window.USER_ROLE === 'editor')) {
        // Pentru status approved, dacă nu există dată, permite setarea unei date
        let defaultValue = '';
        if (data) {
