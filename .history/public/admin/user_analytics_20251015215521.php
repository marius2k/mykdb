...
async function loadContentInteractionData() {
    ...
    try {
        ...
        console.log('Content overview cleared');
        displayUserActionsChart(null);
        console.log('User actions chart cleared');
        displayArticleInteractionTable(null);
        console.log('Article interaction table cleared');
    } catch (initError) {
        console.error('Error during initial display clearing:', initError);
    }
    ...
    try {
        ...
        try {
            console.log('Making API request to get_user_analytics');
            // Add debug=1 parameter for development to bypass authentication requirements
            const apiUrl = `<?= APP_URL ?>public/api/bkd_user_analytics.php?action=get_user_analytics&start_date=${startDate}&end_date=${endDate}&debug=1`;
            ...
            console.log('Data structure check:', {
                ...
                hasOverallStats: 'overall_stats' in data,
                hasDailyActivity: 'daily_activity' in data,
                hasTopArticles: 'top_articles' in data,
                topArticlesIsArray: Array.isArray(data.top_articles),
                topArticlesLength: data.top_articles ? data.top_articles.length : 'n/a'
            });
            
            if (data && data.success === true) {
                // Update the UI with the data - add safety checks and defaults
                try {
                    displayContentInteractionOverview(data.overall_stats || {});
                    console.log('Content overview updated');
                } catch (e) {
                    console.error('Error updating content overview:', e);
                }
                
                try {
                    displayUserActionsChart(data.daily_activity || []);
                    console.log('User actions chart updated');
                } catch (e) {
                    console.error('Error updating user actions chart:', e);
                }
                
                try {
                    // Ensure we have a valid array for the article table
                    const topArticles = Array.isArray(data.top_articles) ? data.top_articles : [];
                    displayArticleInteractionTable(topArticles);
                    console.log('Article interaction table updated with', topArticles.length, 'articles');
                } catch (e) {
                    console.error('Error updating article interaction table:', e);
                }
...
function displayArticleInteractionTable(articles) {
    ...
    try {
        // Track function execution with more detailed logs
        console.log('Starting displayArticleInteractionTable with articles:', articles ? 'data present' : 'no data');
        ...
        try {
            ...
            // No need for a nested try block here
            ...
            try {
                ...
                console.error('Error in DataTable fallback:', fallbackError);
            }
        }
    } catch (error) {
