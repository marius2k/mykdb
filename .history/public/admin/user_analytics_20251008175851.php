...
// Check if the user analytics tables exist
$db = new Database();
$tableExists = false;
try {
    $result = $db->query("SHOW TABLES LIKE 'user_activity_analytics'");
...
async function checkTablesExist() {
    try {
        const response = await fetch(`<?= APP_URL ?>public/api/bkd_user_analytics.php?action=check_tables_exist`);
        if (!response.ok) {
            return false;
        }
