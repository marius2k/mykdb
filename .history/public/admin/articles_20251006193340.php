...
function generatePublishAtForArticleWithVersionData(articleId, versionData, rowData) {
    ...
    return buildPublishAtHtml(articleId, versionData.status, versionData.publish_at || '', versionData.version_number, onlineVersion);
}
...
function renderArticlesTable(articles) {
    ...
    articles.forEach(a => {
        ...
        if (a.status === 'approved' && a.publish_at) {
        ...
        } else if (a.status === 'approved' && (window.USER_ROLE === 'moderator' || window.USER_ROLE === 'admin' || window.USER_ROLE === 'editor')) {
            // Pentru status approved fără dată, permite setarea datei
            ...
            // Pentru orice alt status (pending, draft, published, disabled), nu afișa nimic
            publishAtHtml = `<span>-</span>`;
        }
