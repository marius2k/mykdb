...
function generatePublishAtForArticle(row) {
    ...
    // DEBUG: Verifică datele pentru articolul problematic
    console.log(`🔍 DEBUG generatePublishAtForArticle - Article ${row.article_id}, Version ${selectedVersion}:`, {
        selectedVersionData: selectedVersionData,
        versionStatus: versionStatus,
...
function buildPublishAtHtml(articleId, status, publishAt, version = null, onlineVersion = null, isOnline = null) {
    ...
    const isSelectedVersionOnline = version && onlineVersion && parseInt(version) === parseInt(onlineVersion);
    // Folosește câmpul is_online direct din versiune pentru o verificare precisă
    // Dacă isOnline este null, folosește fallback la logica veche
    const versionIsOnline = isOnline !== null ? (isOnline == 1) : isSelectedVersionOnline;
    
    // DEBUG: Afișează informații pentru a diagnostica problema
    if (articleId && version) {
        console.log(`🔍 DEBUG buildPublishAtHtml - Article ${articleId}, Version ${version}:`, {
            status: status,
            isOnline: isOnline,
            versionIsOnline: versionIsOnline,
    ...
    // Verifică explicit că versiunea NU este online (is_online=0) și este approved
    if ((status || '') === 'approved' && !versionIsOnline) {
        // Pentru versiuni approved offline, permite întotdeauna editarea datei
        if (window.USER_ROLE === 'moderator' || window.USER_ROLE === 'admin' || window.USER_ROLE === 'editor') {
            ...
            if (!defaultValue) {
                ...
                defaultValue = data.replace(' ', 'T').substring(0, 16);
            }
            ...
            if (data) {
                ...
                const now = new Date();
                if (pubDate > now) {
                    return `<span style="color: #e67e22;" title="Programat pentru publicare">${data}</span>`;
                } else {
    ...
    else if ((status || '') === 'disabled' && versionIsOnline) {
        if (data) {
            ...
            if (window.USER_ROLE === 'moderator' || window.USER_ROLE === 'admin' || window.USER_ROLE === 'editor') {
                ...
                return `<input type="datetime-local" value="${defaultValue}" onchange="changePublishAt(this.value, ${articleId}, ${version})">`;
            } else {
                return `<span>-</span>`;
            }
        }
