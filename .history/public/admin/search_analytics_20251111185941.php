...
async function loadUserBehaviorData(period = currentPeriod) {
    try {
        ...
        // Debug: Check if article_id is now populated
        if (data.length > 0) {
            console.log('First 3 records with article_id:', data.slice(0, 3).map(d => ({
                query: d.query,
                article_id: d.article_id,
                article_title: d.article_title
            })));
            const withArticles = data.filter(d => d.article_id > 0);
            console.log(`Records with article_id > 0: ${withArticles.length}`);
        }
