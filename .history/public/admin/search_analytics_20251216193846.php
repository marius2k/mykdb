...
<div class="breadcrumb-filter-section" style="display: flex; justify-content: space-between; align-items: center; width: 100vw; padding: 6px 20px; margin-top: 0; margin-bottom: 0; margin-left: calc(-50vw + 50%);">
    ...
    <div style="display: flex; align-items: center; gap: 8px; margin-right: 20px;">
        <label for="period-selector" style="color: #666; font-size: 14px; margin: 0;"><?= lang('lang_search_analytics_time_period') ?>:</label>
        <select id="period-selector" class="form-control" style="width: 140px; height: 32px; font-size: 14px; padding: 4px 8px; margin: 0; border: 1px solid #ccc; border-radius: 4px;">
...
<div class="container" style="max-width: 1400px; margin: 0 auto;">
    ...
    <div style="margin-bottom: 20px;">
        <!-- Section Tabs -->
        <div class="analytics-tabs" style="margin: 0; border-bottom: none;">
    ...
    <div class="section-content active" id="overview-section">
        ...
        <!-- User Search Statistics wrapped in CardInfoBox -->
        <div id="user-search-stats-root"></div>
...
// Render functions for CardInfoBox components in Search Analytics
function renderSearchTrendsCard() {
    const root = ReactDOM.createRoot(document.getElementById('search-trends-root'));
    ...
    root.render(
        React.createElement(CardInfoBox, {
            ...
            title: '<?= lang('lang_search_analytics_search_trends') ?>',
            body: bodyContent,
            defaultOpen: false
        })
    );
...
function renderUserSearchStatsCard() {
    const root = ReactDOM.createRoot(document.getElementById('user-search-stats-root'));
    
    const bodyContent = React.createElement('div', null,
        React.createElement('p', {
            style: { color: '#666', fontSize: '0.9em' }
        }, '<?= lang('lang_search_analytics_search_activity_desc') ?>'),
        React.createElement('div', {
            id: 'user-search-stats-container'
...
function displayUserSearchStats(userStats) {
    ...
    let html = `
        <table id="user-search-stats-table" class="articles-table" style="font-size: 0.85em; width:100%">
            <thead>
                <tr>
                    <th>${TRANSLATIONS.table_user}</th>
                    <th>${TRANSLATIONS.table_role}</th>
                    <th style="text-align:center;">${TRANSLATIONS.table_total_searches}</th>
                    <th style="text-align:center;">${TRANSLATIONS.table_unique_queries}</th>
                    <th style="text-align:center;">${TRANSLATIONS.table_avg_results}</th>
                    <th style="text-align:center;">${TRANSLATIONS.table_clicks}</th>
                    <th style="text-align:center;">${TRANSLATIONS.table_articles_clicked}</th>
                    <th style="text-align:center;">${TRANSLATIONS.table_ctr}</th>
                </tr>
            </thead>
            <tbody>`;
    ...
    userStats.forEach(user => {
        ...
        const roleBadge = user.role ? `<span class="badge badge-info">${escapeHtml(user.role)}</span>` : `<span class="badge" style="background: #95a5a6; color: white;">${TRANSLATIONS.guest}</span>`;
        ...
        if (hasClick && activity.article_title) {
            html += `
                    <div style="color: #7f8c8d; font-size: 0.9em;">
                        → ${TRANSLATIONS.clicked} <a href="<?= APP_URL ?>public/view_article.php?id=${activity.article_id}" target="_blank">${escapeHtml(activity.article_title)}</a>
                        ${activity.position > 0 ? `${TRANSLATIONS.at_position} #${activity.position}` : ''}
                    </div>`;
        } else {
            html += `
                    <div style="color: #95a5a6; font-size: 0.9em;">
                        ${TRANSLATIONS.no_click_recorded} (${activity.result_count || 0} ${TRANSLATIONS.results_shown})
                    </div>`;
        }
