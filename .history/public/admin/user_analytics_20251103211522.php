...
<div class="analytics-container" >
    ...
    <div class="tab-content" id="content-interaction-tab">
        ...
        <div style="margin-top: 30px;">
            <h4 style="font-size: 1.5em; margin-bottom: 20px;"><img src="<?=APP_URL?>assets/icons/icon-search.svg" width="40">&nbsp;&nbsp;Search Behavior</h4>
            
            <!-- Search Metrics Cards -->
            <div class="stats-grid">
                <div class="stat-card" style="border-left-color: #9b59b6;">
                    <div class="stat-value" id="search-total-searches">0</div>
                    <div class="stat-label">Total Searches</div>
                </div>
                <div class="stat-card" style="border-left-color: #3498db;">
                    <div class="stat-value" id="search-unique-queries">0</div>
                    <div class="stat-label">Unique Queries</div>
                </div>
                <div class="stat-card" style="border-left-color: #2ecc71;">
                    <div class="stat-value" id="search-ctr">0%</div>
                    <div class="stat-label">Click-Through Rate</div>
                </div>
                <div class="stat-card" style="border-left-color: #f39c12;">
                    <div class="stat-value" id="search-avg-results">0</div>
                    <div class="stat-label">Avg Results per Search</div>
                </div>
            </div>
            
            <!-- Top Search Queries -->
            <div class="chart-container" style="margin-top: 20px;">
                <h4 style="font-size: 1.2em; margin-bottom: 15px;">🔍 Top Search Queries</h4>
                <div id="top-search-queries-container">
            ...
            <div class="chart-container" style="margin-top: 20px;">
                <h4 style="font-size: 1.2em; margin-bottom: 15px;">🎯 Most Clicked Articles from Search</h4>
                <div id="clicked-articles-from-search-container">
                    <p style="color: #999; text-align: center; padding: 20px;">Loading...</p>
                </div>
            </div>
            ...
            <div class="chart-container" style="margin-top: 20px;">
                <h4 style="font-size: 1.2em; margin-bottom: 15px;">📊 Recent Search Activity</h4>
                <div id="recent-search-activity-container" style="max-height: 400px; overflow-y: auto;">
                    <p style="color: #999; text-align: center; padding: 20px;">Loading...</p>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadSearchBehaviorData(); // Load search behavior data on initial load
    ...
    document.getElementById('update-filters').addEventListener('click', function() {
        ...
        switch (activeTab) {
            case 'user-engagement':
                loadUserEngagementData();
                loadSearchBehaviorData(); // Load search behavior data
                break;
            case 'admin-activity':
...
// Load and display search behavior data
async function loadSearchBehaviorData() {
    try {
        const startDate = document.getElementById('start-date').value;
        const endDate = document.getElementById('end-date').value;
        const roleFilter = document.getElementById('role-filter').value;
        
        console.log('Loading search behavior data...');
        
        // Fetch search queries and clicks
        const response = await fetch(`${APP_URL}public/api/bkd_user_analytics.php?action=search_behavior&start_date=${startDate}&end_date=${endDate}&role=${roleFilter}`);
        ...
        console.log('Search behavior response:', result);
        
        if (!result.success) {
            throw new Error(result.error || 'Failed to load search behavior data');
        }
        
        const data = result.data || [];
        console.log(`Loaded ${data.length} search behavior records`);
        
        // Destroy existing DataTable if exists
        if ($.fn.DataTable.isDataTable('#searchBehaviorTable')) {
            $('#searchBehaviorTable').DataTable().destroy();
        }
        
        $('#searchBehaviorTable tbody').empty();
        
        // Create DataTable with search behavior data
        $('#searchBehaviorTable').DataTable({
            data: data,
            responsive: true,
            ...
            language: {
                url: "https://cdn.datatables.net/plug-ins/1.13.7/i18n/<?= $lang ?>.json",
                emptyTable: USER_ANALYTICS_TRANSLATIONS.no_data || 'No search data available'
            },
            columns: [
                { 
                    data: 'action_date',
                ...
                { 
                    ...
                    render: function(data, type, row) {
                        return data && data !== 'N/A' ? `<strong>${data}</strong>` : '-';
                    }
                },
                { 
                    data: 'result_count',
                ...
                { 
                    data: 'article_title',
                    render: function(data, type, row) {
                        if (data && row.article_id && row.article_id > 0) {
                            return `<a href="${APP_URL}public/view_article.php?id=${row.article_id}" target="_blank">${truncateText(data, 50)}</a>`;
                        }
                        return '-';
                    }
                },
        ...
        console.log('Search behavior table initialized successfully');
        
    } catch (error) {
        console.error('Error loading search behavior:', error);
        showError('Error loading search behavior data: ' + error.message);
    }
}
