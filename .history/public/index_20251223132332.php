...
// Helper functions for Select2 formatting (global scope)
window.truncateText = function(text, maxLength = 15) {
...
window.formatWithIcon = function(state) {
  console.log('formatWithIcon called:', state); // Debug
  ...
  // Force truncation to 15 characters
  const label = window.truncateText(rawText, 15);
  console.log('Formatting:', rawText, '→', label); // Debug log
  const img = $(element).data('img');

  if (img) {
    return $('<span style="display: flex; align-items: center;"><img src="' + img + '" style="width: 20px; height: 20px; margin-right: 8px; flex-shrink: 0;" /><span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="' + rawText + '">' + label + '</span></span>');
  }
  
  return $('<span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="' + rawText + '">' + label + '</span>');
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        console.log('Initializing Select2...'); // Debug
        ...
        $select.select2({
            ...
            allowClear: true,
            width: '100%',
            templateResult: window.formatWithIcon,
            templateSelection: window.formatWithIcon          
        });
        $select.on('change', triggerAdvancedSearch);
        console.log('Select2 initialized with formatWithIcon'); // Debug
    }, 200);
