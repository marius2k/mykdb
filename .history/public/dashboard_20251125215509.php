...
// Get articles statistics for this week (total, not just user's)
$weekStart = date('Y-m-d', strtotime('monday this week'));

// Published this week (online versions in articles table)
$publishedStmt = $db->query("SELECT COUNT(*) FROM articles WHERE status = 'published' AND created_at >= ?", [$weekStart]);
$publishedCount = $publishedStmt->fetchColumn();

// Drafts this week (from article_versions)
$draftsStmt = $db->query("SELECT COUNT(*) FROM article_versions WHERE status = 'draft' AND created_at >= ?", [$weekStart]);
$draftsCount = $draftsStmt->fetchColumn();

// Pending this week (from article_versions)
$pendingStmt = $db->query("SELECT COUNT(*) FROM article_versions WHERE status = 'pending' AND created_at >= ?", [$weekStart]);
$pendingCount = $pendingStmt->fetchColumn();

// Approved this week (from article_versions, offline approved versions)
$approvedStmt = $db->query("SELECT COUNT(*) FROM article_versions WHERE status = 'approved' AND is_online = 0 AND created_at >= ?", [$weekStart]);
...
<div class="breadcrumb-filter-section" style="display: flex; justify-content: space-between; align-items: center; width: 100vw; padding: 6px 20px; margin-top: 0; margin-bottom: 0; margin-left: calc(-50vw + 50%);">
    <div style="display: flex; gap: 15px; align-items: center; margin-left: 20px; color: #666; font-size: 14px;">
        <?php if ($lastLogin): ?>
            ...
            <span>|</span>
        <?php endif; ?>
        <span><?= lang('lang_articles_this_week') ?>: 
