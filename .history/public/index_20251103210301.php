...
<div class="main" style="display: flex; flex-direction: row; gap: 20px;">
    ...
    <div class="rightSide" style="width: 25%; display: flex; flex-direction: column; gap: 20px;">
            ...
            <div class="search-form" >
                    ...
                    <!-- 🔧 ADVANCED SEARCH -->
                   <div id="advancedSearchForm" style="display: none;  background-color: white; padding-bottom: 30px;">
                          <div style="display: flex; justify-content: center; align-items: center; gap: 12px;">
                            <!-- Search Text Field -->
                            <label for="liveSearch" style="text-align: right; min-width: 80px;"><?= lang('lang_text') ?></label>
                            <input type="text" id="liveSearch" placeholder="<?= lang('lang_search_placeholder') ?>" autocomplete="off" style="flex: 1; max-width: 220px;">
                            <br>
                            </div>
                    </div>
            </div>
            ...
            <div class="search-form">
                ...
                <!-- Filter form -->
                  <div id="advancedFilterForm" style="display: none; padding: 20px; background-color: white; padding-bottom: 30px;">
                   
                    <form id="filterOnCategory" width="100%">
                        <div style="display: flex; flex-direction: column; gap: 12px;">
                            <!-- Author Filter -->
                            <div style="display: flex; justify-content: center; align-items: center; gap: 12px;">
                                <label for="searchAuthor" style="text-align: right; min-width: 80px;"><?= lang('lang_art_author') ?>:</label>
                                <input type="text" id="searchAuthor" name="author" style="flex: 1; max-width: 300px;">
                            </div>

                            <!-- Category Filter -->
                            <div style="display: flex; justify-content: center; align-items: center; gap: 12px;">
                                <label for="filterCategory" style="text-align: right; min-width: 80px;"><?= lang('lang_art_category') ?>:</label>
                                <select id="filterCategory" name="fcategory" class="select2-category" style="flex: 1; max-width: 300px;">
                                    <option value=""></option>
...
advToggleSearch.addEventListener('click', () => {
  ...
  // Clear search form when closing
  if (isOpenSearch) {
    clearSearchForm();
    defaultContent.style.display = 'block';
...
advToggleFilter.addEventListener('click', () => {
  ...
  // Clear filter form when closing
  if (isOpenFilter) {
    document.getElementById('searchAuthor').value = '';
    $('#filterCategory').val(null).trigger('change');
    defaultContent.style.display = 'block';
  ...
  fetch('api/bkd_search_articles.php?' + params.toString())
    ...
    .then(text => {
        ...
        try {
            ...
            searchResults.innerHTML = data.map((article, index) => `
                <div class="article-card search-result">
                  <h4><img src="<?=APP_URL?>/assets/icons/categories/${article.icon}" width="40" height="auto">&nbsp;&nbsp;&nbsp;&nbsp;<a href="view_article.php?id=${article.id}&version=${article.version}" class="search-result-link" data-result-id="${article.id}" data-position="${index + 1}">${highlightQuery(article.title, query)}</a></h4>
                  ...
                  <p><a href="view_article.php?id=${article.id}&version=${article.version}" class="btn btn-sm btn-primary search-result-link" data-result-id="${article.id}" data-position="${index + 1}">Read more...</a></p>
                </div>
...
// Trigger advanced filter (for Author and Category)
function triggerAdvancedFilter() {
  const author = document.getElementById('searchAuthor').value.trim();
  const categoryId = document.getElementById('filterCategory').value;

  const hasFilters = author || categoryId;
  ...
  // Build search query string for analytics
  let searchQuery = '';
  if (author) searchQuery += `author:${author}`;
  if (categoryId) {
      const categoryName = $('#filterCategory option:selected').text();
      searchQuery += (searchQuery ? ' ' : '') + `category:${categoryName}`;
  }

  const params = new URLSearchParams();
  if (author) params.append('author', author);
  if (categoryId) params.append('fcategory', categoryId);
  ...
  fetch('api/bkd_search_articles.php?' + params.toString())
    ...
    .then(text => {
        ...
        try {
            ...
            // Track the filtered search in analytics
            if (typeof SearchAnalytics !== 'undefined' && searchQuery) {
                SearchAnalytics.trackSearch(searchQuery, data.length);
            }
            
            searchResults.innerHTML = data.map((article, index) => `
                <div class="article-card search-result">
                  <h4><img src="<?=APP_URL?>/assets/icons/categories/${article.icon}" width="40" height="auto">&nbsp;&nbsp;&nbsp;&nbsp;<a href="view_article.php?id=${article.id}&version=${article.version}" class="search-result-link" data-result-id="${article.id}" data-position="${index + 1}">${article.title}</a></h4>
                  <p class="article-meta" style="font-size: 0.9rem;"><em><?=lang('lang_art_author')?>: ${article.username} | <?=lang('lang_art_category')?>: ${article.category} | <?=lang('lang_art_publish_at')?>: ${new Date(article.publish_at).toLocaleDateString(window.TZ)}</em></p>
                  <p>${article.content}</p>
                  <p><a href="view_article.php?id=${article.id}&version=${article.version}" class="btn btn-sm btn-primary search-result-link" data-result-id="${article.id}" data-position="${index + 1}">Read more...</a></p>
                </div>
...
// event-uri pentru filtrare
document.getElementById('searchAuthor').addEventListener('input', debounce(triggerAdvancedFilter, 300));
...
$('#filterCategory').on('select2:clear', function () {
  // When category is cleared, check if author is also empty
  const author = document.getElementById('searchAuthor').value.trim();
  
  if (!author) {
    // If both are empty, show default content
    defaultContent.style.display = 'block';
    searchResults.innerHTML = '';
  } else {
    // If author still has value, trigger filter with just author
    triggerAdvancedFilter();
  }
});
