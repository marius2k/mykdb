...
// Published this month (online versions in articles table)
$publishedStmt = $db->query("SELECT COUNT(*) FROM articles WHERE status = 'published' AND created_at >= ?", [$monthStart]);
...
// Approved this month (from article_versions, offline approved versions)
$approvedStmt = $db->query("SELECT COUNT(*) FROM article_versions WHERE status = 'approved' AND is_online = 0 AND created_at >= ?", [$monthStart]);
...
<div class="breadcrumb-filter-section" style="display: flex; justify-content: space-between; align-items: center; width: 100vw; padding: 6px 20px; margin-top: 0; margin-bottom: 0; margin-left: calc(-50vw + 50%);">
    <div style="width: 50%; overflow: hidden; margin-left: 20px; color: #666; font-size: 14px;">
        <div style="display: inline-block; white-space: nowrap; animation: scroll-right 40s linear infinite;">
            <span><?= lang('lang_articles_this_month') ?>: 
                <?= lang('lang_published') ?> <strong><?= $publishedCount ?></strong>, 
                <?= lang('lang_drafts') ?> <strong><?= $draftsCount ?></strong>, 
                <?= lang('lang_pending') ?> <strong><?= $pendingCount ?></strong>, 
                <?= lang('lang_approved') ?> <strong><?= $approvedCount ?></strong>
            </span>
            <span style="margin: 0 40px;"></span>
            <span><?= lang('lang_welcome') ?>, <strong><?= $_SESSION['user']['username'] ?></strong>!</span>
            <span style="margin: 0 40px;"></span>
            <span>📚 Total Articles: <strong><?= $db->query("SELECT COUNT(*) FROM articles")->fetchColumn() ?></strong></span>
            <span style="margin: 0 40px;"></span>
            <span>👥 Total Users: <strong><?= $db->query("SELECT COUNT(*) FROM users")->fetchColumn() ?></strong></span>
