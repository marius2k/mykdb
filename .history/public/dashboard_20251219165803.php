...
// Get last login time from activity_log table
$lastLoginStmt = $db->query("SELECT created_at FROM activity_log WHERE user_id = ? AND action_type = 'login_success' ORDER BY created_at DESC LIMIT 1", [$userId]);
$lastLoginData = $lastLoginStmt->fetch();
$lastLogin = $lastLoginData['created_at'] ?? null;

// Get articles statistics for this month (total, not just user's)
$monthStart = date('Y-m-01'); // First day of current month

// Published this month (articles that were published this month)
...
// Drafts this month (from article_versions)
...
$onlineUsersCount = $onlineUsersStmt->fetchColumn();
...
<link rel="stylesheet" href="<?= APP_URL ?>assets/css/components/ScrollablePanel.css">
<script crossorigin src="https://unpkg.com/react@18/umd/react.production.min.js"></script>
<script crossorigin src="https://unpkg.com/react-dom@18/umd/react-dom.production.min.js"></script>
<script src="<?= APP_URL ?>assets/js/react-components-dist/CardInfoBox.js"></script>
<script src="<?= APP_URL ?>assets/js/react-components-dist/MetricsInfoBox.js"></script>
<script src="<?= APP_URL ?>assets/js/react-components-dist/ScrollablePanel.js"></script>
...
<div class="breadcrumb-filter-section" style="display: flex; justify-content: space-between; align-items: center; width: 100vw; padding: 6px 20px; margin-top: 0; margin-bottom: 0; margin-left: calc(-50vw + 50%);">
    <div style="width: 50%; overflow: hidden; margin-left: 20px; color: #666; font-size: 14px;">
        <div style="display: inline-block; white-space: nowrap; animation: scroll-left 30s linear infinite;">
            <span>
                <span style="color: #04709bcc; padding-right: 10px;"><?= lang('lang_articles_this_month') ?>:</span> 
                <span style="color: #770286ff; padding-right: 10px;"><?= lang('lang_published') ?>: <?= $publishedCount ?> </span>  
                <span style="color: #d60303ff; padding-right: 10px;"><?= lang('lang_drafts') ?>: <?= $draftsCount ?> </span> 
                <span style="color: #02860dff; padding-right: 10px;"><?= lang('lang_pending') ?>: <?= $pendingCount ?> </span> 
                <span style="color: #029affff; padding-right: 20px;"><?= lang('lang_approved') ?>: <?= $approvedCount ?> </span> 
                <span style="color: #04709bcc; padding-right: 20px;">Total Articles:<?= $db->query("SELECT COUNT(*) FROM articles")->fetchColumn() ?> </span> 
                <span style="color: #04709bcc; padding-right: 20px;">Total Users: <?= $db->query("SELECT COUNT(*) FROM users")->fetchColumn() ?> </span> 
                <span style="color: #04709bcc">Online Users: <?= $onlineUsersCount ?></span>
            </span>
            
        </div>
    </div>
    <div style="display: flex; align-items: center; margin-right: 20px; color: #04709bcc; font-size: 14px;">
        <?php if ($lastLogin): ?>
            <span><?= lang('lang_last_login') ?>: <?= date('Y-m-d H:i', strtotime($lastLogin)) ?></span>
        <?php endif; ?>
    </div>
</div>
<style>
...
<div class="scrollable-panels-wrapper">
    ...
    <div id="recent-activity-root" style="flex: 0 0 calc(60% - 10px);"></div>

    <!-- Platform Statistics -->
    <div id="platform-stats-root" style="flex: 0 0 calc(40% - 10px); margin-left: auto;"></div>
</div>
...
$(function() {
    ...
    $.getJSON('api/bkd_dashboard.php', function(data) {
        ...
        // Wait for React components to render before initializing charts
        setTimeout(function() {
...
function approveArticle(articleId, version = 1) {
    ...
    fetch(`api/bkd_article_approve.php`, {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'action=approve&article_id=' + encodeURIComponent(articleId) + '&version=' + encodeURIComponent(version) + '&csrf_token=' + encodeURIComponent(window.CSRF_TOKEN)
    })

    .then(res => {
        console.log('Response status:', res.status); // Debug
        console.log('Response headers:', res.headers); // Debug
        ...
        // Citește răspunsul ca text mai întâi pentru debug
        return res.text();
    })
    .then(text => {
        console.log('Raw response:', text); // Debug - vezi exact ce primești
        
        // Încearcă să parseazi JSON-ul
        try {
            ...
            alert('Server response is not valid JSON: ' + text);
        }
    })
/*
    .then(res => {
...
/**
 * Reject article; status = pending -> draft
 */
function rejectArticle(articleId, version = 1) {

    fetch(`api/bkd_article_reject.php`, {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'action=reject&article_id=' + encodeURIComponent(articleId) + '&version=' + encodeURIComponent(version) + '&csrf_token=' + encodeURIComponent(window.CSRF_TOKEN)
    })

    .then(res => {
        console.log('rejectArticle: Response status:', res.status); // Debug
        console.log('rejectArticle: Response headers:', res.headers); // Debug
    ...
    .then(text => {
        console.log('rejectArticle: Raw response:', text); // Debug - vezi exact ce primești
...
function submitForApproval(articleId, version = 1) {
    ...
    */
    ...
    fetch('api/bkd_article_submit.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
...
function submitArticle2(submitType) {
    ...
    //console.log ("Action data:", formData.get('action'));
