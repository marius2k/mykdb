...
window.formatWithIcon = function(option) {
  console.log('=== formatWithIcon v2.0 CALLED ===');
  try {
    console.log('formatWithIcon called, option:', option);
    
    if (!option.id) {
      console.log('No id, returning text as-is');
      return option.text;
    }
    ...
    console.log('Original text length:', text.length, 'Text:', text);
    ...
    console.log('After truncation:', text);

    if (imgSrc) {
      return $('<span><img src="' + imgSrc + '" style="width:20px;height:20px;margin-right:8px;" />' + text + '</span>');
    }
    
    return $('<span>' + text + '</span>');
  } catch (e) {
    console.error('Error in formatWithIcon:', e);
    return option.text;
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        ...
        console.log('formatWithIcon function exists:', typeof window.formatWithIcon);
        ...
        $select.select2({
            ...
            templateResult: function(option) {
                console.log('templateResult called with:', option);
                console.log('About to call formatWithIcon, function type:', typeof window.formatWithIcon);
                const result = window.formatWithIcon(option);
                console.log('formatWithIcon returned:', result);
                return result;
            },
            templateSelection: function(option) {
                console.log('templateSelection called with:', option);
                const result = window.formatWithIcon(option);
                console.log('formatWithIcon returned:', result);
                return result;
            }
