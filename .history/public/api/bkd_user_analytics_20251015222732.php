...
function getUserAnalytics($db) {
    ...
    try {
        ...
        try {
            // Bookmarks count
            $bookmarks = $db->fetchSingle("SELECT COUNT(*) as count FROM user_activity_analytics WHERE action_type = 'bookmark' $whereClause", $params);
        ...
        try {
            ...
            $usefulnessRatings = $db->fetchSingle("SELECT COUNT(*) as count FROM user_activity_analytics WHERE action_type = 'usefulness_rating' $whereClause", $params);
        ...
        try {
            // Average star rating
            $avgRating = $db->fetchSingle("SELECT AVG(CAST(value AS DECIMAL(3,2))) as avg FROM user_activity_analytics WHERE action_type = 'rating' $whereClause", $params);
        ...
        try {
            // PDF saves count
            $pdfSaves = $db->fetchSingle("SELECT COUNT(*) as count FROM user_activity_analytics WHERE action_type = 'save_pdf' $whereClause", $params);
        ...
        try {
            // Comments count
            $comments = $db->fetchSingle("SELECT COUNT(*) as count FROM user_activity_analytics WHERE action_type = 'comment' $whereClause", $params);
        ...
        try {
            // Useful yes count
            $usefulYes = $db->fetchSingle("SELECT COUNT(*) as count FROM user_activity_analytics WHERE action_type = 'usefulness_rating' AND value = 1 $whereClause", $params);
        ...
        try {
            // Useful no count
            $usefulNo = $db->fetchSingle("SELECT COUNT(*) as count FROM user_activity_analytics WHERE action_type = 'usefulness_rating' AND value = 0 $whereClause", $params);
        ...
        try {
            ...
            // First check if we have any articles with interactions
                ...
                try {
                    ...
                    $topArticles = $db->fetchAll(
                        "SELECT 
                            ua.article_id,
                            '' as title,
                            COUNT(DISTINCT CASE WHEN ua.action_type = 'bookmark' THEN 1 END) as bookmarks,
                            COUNT(DISTINCT CASE WHEN ua.action_type = 'save_pdf' THEN 1 END) as pdf_saves,
                            COUNT(DISTINCT CASE WHEN ua.action_type = 'comment' THEN 1 END) as comments,
                            0 as avg_rating,
                            COUNT(DISTINCT CASE WHEN ua.action_type = 'usefulness_rating' AND ua.value = 1 THEN 1 END) as useful_yes,
                            COUNT(DISTINCT CASE WHEN ua.action_type = 'usefulness_rating' AND ua.value = 0 THEN 1 END) as useful_no
                        FROM user_activity_analytics ua
                        $whereClause
                        GROUP BY ua.article_id
                        ORDER BY COUNT(*) DESC
