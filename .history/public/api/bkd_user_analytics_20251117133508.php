...
function getActiveContributors($db) {
    try {
        ...
        $query = "
            ...
            WHERE r.name IN ('contributor', 'editor', 'moderator', 'admin', 'superadmin')
                $roleCondition
        ...
        error_log("getActiveContributors query: " . $query);
...
function getTopCommenters($db) {
    try {
        ...
        $query = "
            ...
            FROM users u
            LEFT JOIN roles r ON u.role_id = r.id
            INNER JOIN article_comments c ON c.user_id = u.id
...
function getEngagedReaders($db) {
    try {
        ...
        $query = "
            ...
            WHERE r.name IN ('user', 'contributor', 'editor', 'moderator', 'admin', 'superadmin')
                $roleCondition
            GROUP BY u.id, u.username, r.name
