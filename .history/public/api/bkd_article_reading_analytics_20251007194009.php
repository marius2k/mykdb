...
function getAnalyticsDashboard($db) {
    ...
    error_log("User data: " . json_encode($_SESSION['user'] ?? 'No user'));
    ...
    if (!in_array($_SESSION['user']['role'], ['admin', 'moderator', 'superadmin'])) {
        throw new Exception('Acces neautorizat - rol: ' . ($_SESSION['user']['role'] ?? 'nedefinit'));
    }
    ...
    // Top articole cu statistici de bază
    ...
    $weeklyStats = $db->fetchSingle("
        SELECT 
            -- Views statistics
            (SELECT COUNT(*) FROM article_views WHERE created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)) as weekly_views,
            (SELECT COUNT(DISTINCT article_id) FROM article_views WHERE created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)) as articles_viewed,
            -- Likes statistics  
