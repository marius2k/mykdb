...
function getArticleWithPermissionCheck($articleId, $action, $version = null) {
    ...
    if ($version) {
        ...
        // Pentru versiuni specifice, determină statusul corect pentru permisiuni
        if ($isOnlineVersion) {
        ...
        error_log("getArticleWithPermissionCheck: Version data - version=$version, status=" . $articleData['status'] . ", is_online=" . $articleData['is_online'] . ", permission_status=" . $articleData['permission_status']);
        ...
        if (!$articleData) {
            ...
            if (!$articleData) {
                error_log("getArticleWithPermissionCheck: Article $articleId not found in any table");
                return null;
            }
            $isOnlineVersion = true;
            $articleData['permission_status'] = $articleData['status']; // din articles
        } else {
            $isOnlineVersion = (int)$articleData['is_online'] === 1;
        ...
        error_log("getArticleWithPermissionCheck: Default version data - version=" . ($articleData['version_number'] ?? 'N/A') . ", status=" . ($articleData['status'] ?? 'N/A') . ", is_online=" . ($articleData['is_online'] ?? 'N/A') . ", permission_status=" . $articleData['permission_status']);
    }
    ...
    error_log("getArticleWithPermissionCheck DEBUG: articleId=$articleId, action=$action");
    error_log("getArticleWithPermissionCheck DEBUG: version_status=" . ($articleData['status'] ?? 'N/A'));
    error_log("getArticleWithPermissionCheck DEBUG: is_online=" . ($articleData['is_online'] ?? 'N/A'));
    error_log("getArticleWithPermissionCheck DEBUG: articles_status=" . ($articleData['articles_status'] ?? 'N/A'));
    error_log("getArticleWithPermissionCheck DEBUG: permission_status=" . ($articleData['permission_status'] ?? 'N/A'));
    error_log("getArticleWithPermissionCheck DEBUG: isOnlineVersion=" . ($isOnlineVersion ? 'true' : 'false'));
    error_log("getArticleWithPermissionCheck DEBUG: statusForPermission=$statusForPermission");

    error_log("getArticleWithPermissionCheck: Checking permissions - statusForPermission=$statusForPermission, isOnlineVersion=" . ($isOnlineVersion ? 'true' : 'false'));
