...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    // Debug temporar - activează logging detaliat
    error_reporting(E_ALL);
    ini_set('display_errors', 0); // Nu afișa erorile în output pentru a nu corupe JSON
    ...
    try {
        ...
        error_log("Draw: $draw, Start: $start, Length: $length, Search: '$searchValue'");

        $userRole = $_SESSION['user']['role'] ?? '';
        $currentUserId = $_SESSION['user']['id'] ?? 0;
        
        error_log("User Role: $userRole, User ID: $currentUserId");
        ...
        if ($userRole === 'contributor') {
            $roleFilter = " AND (av.status = 'approved' OR av.status = 'disabled' OR (av.author_id = :current_user_id AND av.status IN ('draft', 'pending')))";
            $roleParams[':current_user_id'] = $currentUserId;
            error_log("Applied contributor filter: $roleFilter");
        }

        $where = '';
        $params = [];
        if ($searchValue) {
            $where = "AND (av.title LIKE :search1 OR u.username LIKE :search2 OR c.name LIKE :search3)";
            $params = [
                ':search1' => "%$searchValue%",
                ':search2' => "%$searchValue%",
                ':search3' => "%$searchValue%"
            ];
            error_log("Applied search filter: $where");
        }
        ...
        $totalStmt = $db->query("SELECT COUNT(DISTINCT article_id) FROM article_versions");
        ...
        foreach ($params as $k => $v) {
            ...
            error_log("Binding parameter $k = $v");
        }
        ...
        foreach ($params as $k => $v) {
            $stmt->bindValue($k, $v);
            error_log("Binding main query parameter $k = $v");
        }
        
        $stmt->bindValue(':limit', $length, PDO::PARAM_INT);
        $stmt->bindValue(':offset', $start, PDO::PARAM_INT);
        error_log("Binding LIMIT: $length, OFFSET: $start");
        ...
        foreach ($articles as &$a) {
            ...
            error_log("Processed article ID: " . $a['article_id'] . ", Title: " . $a['title'] . ", Status: " . $a['status']);
        }
