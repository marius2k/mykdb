...
function getArticleAnalytics($db) {
    ...
    if (!in_array($period, [7, 30, 90])) {
        $period = 7; // Fallback to default if invalid
    }
    ...
    $stats = $db->fetchSingle("
        SELECT 
            ...
            (SELECT COUNT(*) FROM article_views WHERE article_id = a.id AND viewed_at >= DATE_SUB(NOW(), INTERVAL ? DAY)) as total_views,
            -- Likes din tabelul existent  
            (SELECT COUNT(*) FROM article_likes WHERE article_id = a.id AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY)) as total_likes
        FROM articles a
        WHERE a.id = ?
    ", [$period, $period, $articleId]);
    ...
    if ($hasReadingTime) {
        try {
            $readingStats = $db->fetchSingle("
                ...
                WHERE article_id = ? AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY)
            ", [$articleId, $period]);
...
function getAnalyticsDashboard($db) {
    ...
    if (!in_array($_SESSION['user']['role'], ['admin', 'moderator', 'editor', 'superadmin'])) {
    ...
    $topArticles = $db->fetchAll("
        SELECT 
            ...
            (SELECT COUNT(*) FROM article_views WHERE article_id = a.id AND viewed_at >= DATE_SUB(NOW(), INTERVAL ? DAY)) as total_views,
            -- Public views only
            (SELECT COUNT(*) FROM article_views WHERE article_id = a.id AND viewed_at >= DATE_SUB(NOW(), INTERVAL ? DAY) AND (view_source = 'public' OR view_source IS NULL)) as public_views,
            -- Admin views only
            (SELECT COUNT(*) FROM article_views WHERE article_id = a.id AND viewed_at >= DATE_SUB(NOW(), INTERVAL ? DAY) AND view_source IN ('admin_preview', 'admin_edit', 'admin_analytics')) as admin_views,
            -- Likes din tabelul existent
            (SELECT COUNT(*) FROM article_likes WHERE article_id = a.id AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY)) as total_likes
        FROM articles a
        ...
        try {
            foreach ($topArticles as &$article) {
                ...
                if ($readingStats) {
                    ...
                    // Add new public/admin specific stats
                    $article['avg_public_reading_time'] = $readingStats['avg_public_reading_time'] ?: 0;
                    $article['avg_admin_reading_time'] = $readingStats['avg_admin_reading_time'] ?: 0;
                    $article['public_reading_sessions'] = $readingStats['public_reading_sessions'] ?: 0;
                    $article['admin_reading_sessions'] = $readingStats['admin_reading_sessions'] ?: 0;
                } else {
                    $article['avg_reading_time'] = 0;
                    $article['avg_scroll_percentage'] = 0;
                    $article['unique_readers'] = 0;
                    $article['reading_sessions'] = 0;
                    $article['avg_public_reading_time'] = 0;
                    $article['avg_admin_reading_time'] = 0;
                    $article['public_reading_sessions'] = 0;
                    $article['admin_reading_sessions'] = 0;
                }
                
                $article['engagement_score'] = $article['public_views'] + $article['total_likes'] + $article['public_reading_sessions'];
            }
        } catch (Exception $e) {
            ...
            foreach ($topArticles as &$article) {
                ...
                $article['public_reading_sessions'] = 0;
                $article['admin_reading_sessions'] = 0;
                $article['engagement_score'] = $article['public_views'] + $article['total_likes'];
            }
        }
    } else {
        // Setează valori default pentru reading time
    ...
    // Statistici generale - separate public/admin views
    $weeklyStats = $db->fetchSingle("
        SELECT 
            -- Total views statistics (using viewed_at column)
            (SELECT COUNT(*) FROM article_views WHERE viewed_at >= DATE_SUB(NOW(), INTERVAL ? DAY)) as weekly_views,
            -- Public views only
            (SELECT COUNT(*) FROM article_views WHERE viewed_at >= DATE_SUB(NOW(), INTERVAL ? DAY) AND (view_source = 'public' OR view_source IS NULL)) as weekly_public_views,
            -- Admin views only
            (SELECT COUNT(*) FROM article_views WHERE viewed_at >= DATE_SUB(NOW(), INTERVAL ? DAY) AND view_source IN ('admin_preview', 'admin_edit', 'admin_analytics')) as weekly_admin_views,
            (SELECT COUNT(DISTINCT article_id) FROM article_views WHERE viewed_at >= DATE_SUB(NOW(), INTERVAL ? DAY)) as articles_viewed,
            -- Likes statistics (using created_at column)
            (SELECT COUNT(*) FROM article_likes WHERE created_at >= DATE_SUB(NOW(), INTERVAL ? DAY)) as weekly_likes
        ...
        try {
            $readingWeeklyStats = $db->fetchSingle("
                SELECT 
                    ...
                    COUNT(DISTINCT COALESCE(user_id, session_id)) as unique_readers,
                    -- Separate public/admin reading stats
                    COUNT(CASE WHEN (view_source = 'public' OR view_source IS NULL) THEN 1 END) as weekly_public_reading_sessions,
                    COUNT(CASE WHEN view_source IN ('admin_preview', 'admin_edit', 'admin_analytics') THEN 1 END) as weekly_admin_reading_sessions,
                    AVG(CASE WHEN (view_source = 'public' OR view_source IS NULL) THEN reading_time END) as avg_weekly_public_reading_time,
                    AVG(CASE WHEN view_source IN ('admin_preview', 'admin_edit', 'admin_analytics') THEN reading_time END) as avg_weekly_admin_reading_time
                FROM article_reading_time 
            ...
            $weeklyStats['weekly_public_reading_sessions'] = 0;
            $weeklyStats['weekly_admin_reading_sessions'] = 0;
            $weeklyStats['avg_weekly_public_reading_time'] = 0;
            $weeklyStats['avg_weekly_admin_reading_time'] = 0;
        }
    } else {
        // Setează valori default pentru reading time
        $weeklyStats['weekly_reading_sessions'] = 0;
...
function getCombinedStats($db) {
    ...
    $articles = $db->fetchAll("
        ...
        LIMIT ?
    ", [$period, $period, $period, $period, $limit * 2]); // Luăm mai multe pentru a putea sorta după
        ...
        try {
            foreach ($articles as &$article) {
                $readingStats = $db->fetchSingle("
                    SELECT 
                        AVG(reading_time) as avg_reading_time,
                        -- Public reading time only (view_source = 'public' OR NULL)
                        (SELECT AVG(reading_time) FROM article_reading_time 
                         WHERE article_id = ? AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY) AND (view_source = 'public' OR view_source IS NULL)) as avg_public_reading_time,
                        -- Admin reading time only
                        (SELECT AVG(reading_time) FROM article_reading_time 
                         WHERE article_id = ? AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY) AND view_source IN ('admin_preview', 'admin_edit', 'admin_analytics')) as avg_admin_reading_time,
                        AVG(scroll_percentage) as avg_scroll_percentage,
                        COUNT(DISTINCT COALESCE(user_id, session_id)) as unique_readers,
                        COUNT(*) as reading_sessions
                    FROM article_reading_time 
                    WHERE article_id = ? AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY)
                ", [$article['id'], $period, $article['id'], $period, $article['id'], $period]);
