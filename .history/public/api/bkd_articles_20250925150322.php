...
function getArticleWithPermissionCheck($articleId, $action, $version = null) {
    ...
    if ($version) {
        ...
        // IMPORTANT: Pentru articole cu is_online = 1, verifică statusul din tabela articles
        if ($isOnlineVersion) {
            ...
            if ($articlesStatus) {
                ...
                // Pentru permisiuni, folosește statusul din articles dacă versiunea este online
                $articleData['permission_status'] = $articlesStatus['status'];
            }
        } else {
            // Pentru versiuni offline, folosește statusul din article_versions
    ...
    if (isset($_GET['action'])) {
        ...
        try {
            ...
            if ($userRole === 'contributor') {
                // Contributor: Approved (toate) + Draft/Pending (proprii)
                $whereConditions[] = "(a.status = 'published' OR a.status = 'disabled' OR (av.author_id = ? AND av.status IN ('draft', 'pending', 'approved')))";
