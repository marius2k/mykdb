...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($versionData === false) {
        http_response_code(403);
        echo json_encode(['error' => 'Nu aveți permisiunea să returnați această versiune la starea Pending']);
        exit;
    }
    if ($versionData === null) {
    ...
    if ($versionData['status'] !== 'approved') {
        echo json_encode(['error' => 'Doar versiunile aprobate pot fi returnate la starea Pending']);
        exit;
    }
    ...
    try {
        ...
        // Update version status from 'approved' to 'pending'
        $db->query("UPDATE article_versions SET status = 'pending', updated_at = NOW(), change_note = CONCAT(COALESCE(change_note, ''), ?) WHERE article_id = ? AND version_number = ?", [$noteMessage, $articleId, $version]);
        ...
        logActivity($user_id, 'unapprove_article', 
            'User '. $username .' returned version '. $version .' of article ID '. $articleId .' to pending status. Reason: ' . $reason);
        
        $db->commit();
        
        echo json_encode([
            'success' => true,
            'message' => 'Versiunea a fost returnată la starea Pending cu succes'
        ]);
