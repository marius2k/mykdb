    ...
    if (isset($_GET['action'])) {
        ...
        try {
            ...
            if ($userRole === 'contributor') {
                // Contributor: approved (toate) + published (toate) + pending (proprii) + draft (proprii)
                // Pentru published - verifică în articles.status = 'published' 
                // Pentru approved/pending/draft - verifică în article_versions.status
                $whereConditions[] = "(a.status = 'published' OR av.status = 'approved' OR (av.author_id = ? AND av.status IN ('draft', 'pending')))";
