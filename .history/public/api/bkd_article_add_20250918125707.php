...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_article') {
        ...
        try {
            ...
            // 1. Salvează în article_versions cu article_id = NULL (va fi populat la primul publish)
            $initial_note = $change_note ?: 'First version';

            $db->insert("article_versions", [
                "article_id" => null,
                "version_number" => 1,
                "title" => $title,
            ...
            //$stmt = $db->prepare("INSERT INTO article_versions (article_id, version_number, title, content, category_id, author_id, status, is_online, created_at, updated_at, change_note) VALUES (NULL, 1, ?, ?, ?, ?, ?, 0, NOW(), NOW(), ?)");
            //$stmt->execute([$title, $clean_content, $category_id, $user_id, $status, $initial_note]);
            
            // 2. Obține ID-ul versiunii create (article_versions.id)
            $version_id = $db->lastInsertedId();
            
            // 3. Salvează tagurile - pentru articole nepublicate folosim un identificator temporar
            // Va fi actualizat cu article_id (id din tabela articles) real la primul publish
            if ($tagList) {
                foreach ($tagList as $tagName) {
                    ...
                    // Pentru articole nepublicate, folosim o convenție temporară
                    // article_id = -version_id (negativ pentru a marca că e temporar)
                    $insertArticleTagStmt = $db->prepare("INSERT INTO article_tags (article_id, tag_id) VALUES (?, ?)");
                    $insertArticleTagStmt->execute([-$version_id, $tagId]);
                }
            }
