...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'edit_article') {
        ...
        if ($isEditingOnlineVersion === 1) {
            ...
            // Track article edit in analytics
            try {
                $tracker = new AnalyticsTracker();
                $tracker->trackEdit($user_id, $articleId);
            } catch (Exception $e) {
                error_log("Failed to track article edit: " . $e->getMessage());
            }
    ...
    if ($action === 'publish') {
        ...
        try {
            ...
            // Track publish in analytics
            try {
                $tracker = new AnalyticsTracker();
                $tracker->trackPublish($user_id, $articleId);
            } catch (Exception $e) {
                error_log("Failed to track article publish: " . $e->getMessage());
            }
