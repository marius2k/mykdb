...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        ...
        if ($userRole === 'contributor') {
            $roleFilter = " AND (av.status = 'approved' OR av.status = 'disabled' OR (av.author_id = ? AND av.status IN ('draft', 'pending')))";
            $roleParams[] = $currentUserId;
        }

        $searchFilter = '';
        $searchParams = [];
        if ($searchValue) {
            $searchFilter = " AND (av.title LIKE ? OR u.username LIKE ? OR c.name LIKE ?)";
            $searchParams = ["%$searchValue%", "%$searchValue%", "%$searchValue%"];
        }
        
        // Combine all parameters
        $allParams = array_merge($searchParams, $roleParams);

        // Total articles count - SIMPLIFICAT
        error_log("=== Getting total count ===");
        $totalStmt = $db->prepare("SELECT COUNT(DISTINCT article_id) FROM article_versions");
        $totalStmt->execute();
        $totalArticles = $totalStmt->fetchColumn();
        error_log("Total articles: $totalArticles");

        // Filtered count - SIMPLIFICAT
        ...
        // Prepare parameters with LIMIT and OFFSET
        $mainParams = array_merge($allParams, [$length, $start]);
        ...
        foreach ($articles as &$a) {
            ...
            $versionsStmt = $db->prepare("SELECT version_number FROM article_versions WHERE article_id = ? ORDER BY version_number DESC");
            $versionsStmt->execute([$a['article_id']]);
            $versions = $versionsStmt->fetchAll(PDO::FETCH_COLUMN);
