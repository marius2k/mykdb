...
// Suprimă erorile PHP pentru a nu corupe JSON-ul
error_reporting(0);
...
try {
    ...
    // Increment view counter - cu try/catch pentru a nu opri execuția
    try {
    ...
    // Fetch tags pentru acest articol - cu protecție împotriva erorilor
    $tags = [];
    try {
        $tags = $db->fetchAll("
            SELECT t.name 
    ...
    // Bookmark check - cu protecție
    $isBookmarked = false;
    try {
        if (!empty($_SESSION['user']['id']) && function_exists('is_article_bookmarked')) {
