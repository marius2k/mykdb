...
function getSearchBehavior($db) {
    ...
    try {
        ...
        // We correlate searches with clicks using session_id and search_id from search_result_clicks
        $searchBehavior = $db->fetchAll(
            "SELECT 
                ua.action_date,
                COALESCE(u.username, CONCAT('Guest-', SUBSTRING(ua.session_id, 1, 8))) as username,
                r.name as role,
                sq.query,
                sq.result_count,
                a.id as article_id,
                a.title as article_title,
                src.position as click_position,
                src.time_to_click
             FROM user_activity_analytics ua
             LEFT JOIN users u ON ua.user_id = u.id
             LEFT JOIN roles r ON u.role_id = r.id
             LEFT JOIN search_queries sq ON ua.session_id = sq.session_id 
                 AND DATE(ua.action_date) = DATE(sq.created_at)
                 AND ua.action_type = 'search'
             LEFT JOIN search_result_clicks src ON sq.id = src.search_id
             LEFT JOIN articles a ON src.article_id = a.id
             WHERE ua.action_date BETWEEN ? AND ?
             AND ua.action_type IN ('search', 'search_click')
             $roleCondition
        ...
        foreach ($searchBehavior as $row) {
            $formattedData[] = [
                ...
                'position' => (int)($row['click_position'] ?? 0),
