...
function getAnalyticsDashboard($db) {
    ...
    $weeklyStats = $db->fetchSingle("
        SELECT 
            ...
            -- Public views only
            (SELECT COUNT(*) FROM article_views WHERE viewed_at >= DATE_SUB(NOW(), INTERVAL 7 DAY) AND (view_source = 'public' OR view_source IS NULL)) as weekly_public_views,
            -- Admin views only
            (SELECT COUNT(*) FROM article_views WHERE viewed_at >= DATE_SUB(NOW(), INTERVAL 7 DAY) AND view_source IN ('admin_preview', 'admin_edit', 'admin_analytics')) as weekly_admin_views,
