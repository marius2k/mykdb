...
try {
        ...
        if (!empty($tag)) {
            $sql .= " AND EXISTS (
                ...
                WHERE at.article_id = a.id AND LOWER(t.name) = :tag
            )";
        ...
        */
        ...
        // Filter out false matches from base64 images
        if (!empty($q)) {
            $results = array_filter($results, function($article) use ($q) {
                // Remove base64 images from content and title
                $cleanContent = removeBase64Images($article['content']);
                $cleanTitle = removeBase64Images($article['title']);
                
                // Check if search term still exists after removing base64
                return (stripos($cleanContent, $q) !== false || stripos($cleanTitle, $q) !== false);
            });
        ...
        if (!empty($q)) {
            foreach ($results as $result) {
                $cleanContent = removeBase64Images($result['content']);
                $pos_content = stripos($cleanContent, $q);
                ...
                if ($pos_content !== false) {
                    ...
                    $context = substr($cleanContent, $context_start, 150);
