...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['action'])) {
        ...
        try {
            ...
            // === STEP 1: Extract PUBLISHED articles (article_id NOT NULL) ===
            error_log("bkd_articles: === Getting published articles ===");
            $publishedQuery = "
                SELECT 
                    av.article_id,
                    av.title, 
            ...
            $publishedStmt = $db->prepare($publishedQuery);
            $publishedStmt->execute($params);
            $publishedArticles = $publishedStmt->fetchAll(PDO::FETCH_ASSOC);
            
            error_log("bkd_articles: Found " . count($publishedArticles) . " published articles");

            // === STEP 2: Extract UNPUBLISHED articles (article_id IS NULL) ===
            error_log("bkd_articles: === Getting unpublished articles ===");
            $unpublishedQuery = "
                SELECT 
                    av.id as article_id,
                    av.title, 
            ...
            $unpublishedStmt = $db->prepare($unpublishedQuery);
            $unpublishedStmt->execute($params);
            $unpublishedArticles = $unpublishedStmt->fetchAll(PDO::FETCH_ASSOC);
            
            error_log("bkd_articles: Found " . count($unpublishedArticles) . " unpublished articles");

            // === STEP 3: Combine and sort by updated_at ===
            $allArticles = array_merge($publishedArticles, $unpublishedArticles);
            
            // Sort combined array by updated_at DESC
            usort($allArticles, function($a, $b) {
                return strtotime($b['updated_at']) - strtotime($a['updated_at']);
            });
            
            error_log("bkd_articles: Combined total: " . count($allArticles) . " articles");

            // === STEP 4: Calculate totals ===
            $totalArticles = count($allArticles);
            $filtered = $totalArticles; // Since we already applied filters
            
            // === STEP 5: Apply pagination ===
            $articles = array_slice($allArticles, $start, $length);
            
            error_log("bkd_articles: After pagination: " . count($articles) . " articles");

            // === STEP 6: Process results - get ALL versions for each article ===
            $rownum = $start + 1;
            foreach ($articles as &$a) {
                $a['rownum'] = $rownum++;
                
                if ($a['article_type'] === 'unpublished') {
                    // Articol nepublicat - găsește versiunile după title și author
                     $versionsStmt = $db->prepare("
                        SELECT version_number, status, is_online, NULL as publish_at
                    ...
                    // Articol publicat - găsește versiunile după article_id și include publish_at pentru versiunea online
                    $versionsStmt = $db->prepare("
                        SELECT 
                            av.version_number, 
                            av.status, 
                ...
                error_log("bkd_articles: Processed article ID: " . $a['article_id'] . " (Type: " . $a['article_type'] . "), Title: " . $a['title'] . ", Current Version: " . $a['current_version'] . ", Is Online: " . $a['is_online'] . ", Total versions: " . count($a['versions']));
            }
