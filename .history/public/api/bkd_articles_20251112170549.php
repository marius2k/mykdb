        ...
        try {
            ...
            $filteredQuery = "
                ...
                INNER JOIN (
                    SELECT 
                        article_id,
                        MAX(CASE WHEN is_online = 1 THEN version_number ELSE NULL END) as online_version,
                        MAX(version_number) as latest_version
                    FROM article_versions
                    WHERE article_id IS NOT NULL
                    GROUP BY article_id
                ) av_sel ON av.article_id = av_sel.article_id 
                    AND av.version_number = COALESCE(av_sel.online_version, av_sel.latest_version)
                LEFT JOIN users u ON av.author_id = u.id
