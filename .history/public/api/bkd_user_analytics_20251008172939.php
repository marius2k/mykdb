<?php
require_once '../../config/bootstrap.php';
require_once APP_ROOT . 'includes/functions.php';

header('Content-Type: application/json');
header('Access-Control-Allow-Origin: *');
header('Access-Control-Allow-Methods: GET, POST, OPTIONS');
header('Access-Control-Allow-Headers: Content-Type');

if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    exit(0);
}

try {
    $db = new Database();
    $action = $_GET['action'] ?? $_POST['action'] ?? '';

    switch ($action) {
        case 'track_user_action':
            trackUserAction($db);
            break;
        case 'get_user_analytics':
            getUserAnalytics($db);
            break;
        case 'get_admin_activity':
            getAdminActivity($db);
            break;
        case 'get_user_engagement':
            getUserEngagement($db);
            break;
        default:
            throw new Exception('Invalid action');
    }
} catch (Exception $e) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => $e->getMessage()]);
}

/**
 * Track various user actions on articles
 */
function trackUserAction($db) {
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        http_response_code(405);
        echo json_encode(['success' => false, 'error' => 'Method not allowed']);
        return;
    }

    $data = json_decode(file_get_contents('php://input'), true);
    
    $articleId = (int)($data['article_id'] ?? 0);
    $actionType = $data['action_type'] ?? '';
    $actionValue = $data['action_value'] ?? null;
    $actionMetadata = $data['action_metadata'] ?? null;
    
    $validActions = [
        'bookmark', 'usefulness_rating', 'star_rating', 
        'pdf_save', 'comment', 'view',
        'admin_edit', 'admin_publish', 'admin_approve'
    ];
    
    if (!in_array($actionType, $validActions)) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Invalid action type']);
        return;
    }
    
    if ($articleId <= 0) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Invalid article ID']);
        return;
    }
    
    // Session is already started by bootstrap.php
    $sessionId = session_id();
    $userId = $_SESSION['user']['id'] ?? null;
    $ipAddress = $_SERVER['REMOTE_ADDR'] ?? '';
    $userAgent = $_SERVER['HTTP_USER_AGENT'] ?? '';
    
    try {
        $metadataJson = $actionMetadata ? json_encode($actionMetadata) : null;
        
        $db->insert('user_analytics', [
            'user_id' => $userId,
            'session_id' => $sessionId,
            'article_id' => $articleId,
            'action_type' => $actionType,
            'action_value' => $actionValue,
            'action_metadata' => $metadataJson,
            'ip_address' => $ipAddress,
            'user_agent' => $userAgent,
            'created_at' => date('Y-m-d H:i:s')
        ]);
        
        echo json_encode(['success' => true]);
    } catch (Exception $e) {
        http_response_code(500);
        echo json_encode(['success' => false, 'error' => $e->getMessage()]);
    }
}

/**
 * Get analytics about user interactions
 */
function getUserAnalytics($db) {
    // Check permissions
    if (!isset($_SESSION['user']) || !in_array($_SESSION['user']['role'], ['admin', 'superadmin'])) {
        http_response_code(403);
        echo json_encode(['success' => false, 'error' => 'Unauthorized']);
        return;
    }

    // Optional filters
    $articleId = (int)($_GET['article_id'] ?? 0);
    $userId = (int)($_GET['user_id'] ?? 0);
    $startDate = $_GET['start_date'] ?? date('Y-m-d', strtotime('-30 days'));
    $endDate = $_GET['end_date'] ?? date('Y-m-d');
    
    $whereConditions = [];
    $params = [];
    
    if ($articleId > 0) {
        $whereConditions[] = 'article_id = ?';
        $params[] = $articleId;
    }
    
    if ($userId > 0) {
        $whereConditions[] = 'user_id = ?';
        $params[] = $userId;
    }
    
    if ($startDate) {
        $whereConditions[] = 'created_at >= ?';
        $params[] = $startDate . ' 00:00:00';
    }
    
    if ($endDate) {
        $whereConditions[] = 'created_at <= ?';
        $params[] = $endDate . ' 23:59:59';
    }
    
    $whereClause = !empty($whereConditions) ? 'WHERE ' . implode(' AND ', $whereConditions) : '';
    
    try {
        // Get overall stats
        $overallStats = [
            'bookmarks' => $db->fetchSingle("SELECT COUNT(*) as count FROM user_analytics WHERE action_type = 'bookmark' $whereClause", $params)['count'] ?? 0,
            'usefulness_ratings' => $db->fetchSingle("SELECT COUNT(*) as count FROM user_analytics WHERE action_type = 'usefulness_rating' $whereClause", $params)['count'] ?? 0,
            'avg_star_rating' => $db->fetchSingle("SELECT AVG(CAST(action_value AS DECIMAL(3,2))) as avg FROM user_analytics WHERE action_type = 'star_rating' $whereClause", $params)['avg'] ?? 0,
            'pdf_saves' => $db->fetchSingle("SELECT COUNT(*) as count FROM user_analytics WHERE action_type = 'pdf_save' $whereClause", $params)['count'] ?? 0,
            'comments' => $db->fetchSingle("SELECT COUNT(*) as count FROM user_analytics WHERE action_type = 'comment' $whereClause", $params)['count'] ?? 0,
        ];
        
        // Get daily activity for charts
        $dailyActivity = $db->fetchAll(
            "SELECT 
                DATE(created_at) as date,
                action_type,
                COUNT(*) as count
             FROM user_analytics
             $whereClause
             GROUP BY DATE(created_at), action_type
             ORDER BY DATE(created_at) DESC
             LIMIT 30", 
            $params
        );
        
        // Top articles by interaction
        $topArticles = $db->fetchAll(
            "SELECT 
                ua.article_id,
                a.title,
                COUNT(DISTINCT CASE WHEN ua.action_type = 'bookmark' THEN ua.id END) as bookmarks,
                COUNT(DISTINCT CASE WHEN ua.action_type = 'pdf_save' THEN ua.id END) as pdf_saves,
                COUNT(DISTINCT CASE WHEN ua.action_type = 'comment' THEN ua.id END) as comments,
                AVG(CASE WHEN ua.action_type = 'star_rating' THEN CAST(ua.action_value AS DECIMAL(3,2)) ELSE NULL END) as avg_rating,
                COUNT(DISTINCT CASE WHEN ua.action_type = 'usefulness_rating' AND ua.action_value = 'yes' THEN ua.id END) as useful_yes,
                COUNT(DISTINCT CASE WHEN ua.action_type = 'usefulness_rating' AND ua.action_value = 'no' THEN ua.id END) as useful_no
             FROM user_analytics ua
             JOIN articles a ON ua.article_id = a.id
             $whereClause
             GROUP BY ua.article_id, a.title
             ORDER BY COUNT(*) DESC
             LIMIT 20", 
            $params
        );
        
        echo json_encode([
            'success' => true,
            'overall_stats' => $overallStats,
            'daily_activity' => $dailyActivity,
            'top_articles' => $topArticles
        ]);
        
    } catch (Exception $e) {
        http_response_code(500);
        echo json_encode(['success' => false, 'error' => $e->getMessage()]);
    }
}

/**
 * Get admin activity analytics
 */
function getAdminActivity($db) {
    // Check permissions
    if (!isset($_SESSION['user']) || !in_array($_SESSION['user']['role'], ['admin', 'superadmin'])) {
        http_response_code(403);
        echo json_encode(['success' => false, 'error' => 'Unauthorized']);
        return;
    }
    
    $startDate = $_GET['start_date'] ?? date('Y-m-d', strtotime('-30 days'));
    $endDate = $_GET['end_date'] ?? date('Y-m-d');
    
    try {
        // Admin activity summary
        $activitySummary = $db->fetchAll(
            "SELECT 
                u.username,
                COUNT(CASE WHEN ua.action_type = 'admin_edit' THEN 1 END) as edits,
                COUNT(CASE WHEN ua.action_type = 'admin_publish' THEN 1 END) as publishes,
                COUNT(CASE WHEN ua.action_type = 'admin_approve' THEN 1 END) as approvals,
                COUNT(*) as total_actions
             FROM user_analytics ua
             JOIN users u ON ua.user_id = u.id
             WHERE ua.action_type IN ('admin_edit', 'admin_publish', 'admin_approve')
                AND ua.created_at BETWEEN ? AND ?
             GROUP BY u.id, u.username
             ORDER BY total_actions DESC",
            [$startDate . ' 00:00:00', $endDate . ' 23:59:59']
        );
        
        // Daily admin activity
        $dailyActivity = $db->fetchAll(
            "SELECT 
                DATE(created_at) as date,
                action_type,
                COUNT(*) as count
             FROM user_analytics
             WHERE action_type IN ('admin_edit', 'admin_publish', 'admin_approve')
                AND created_at BETWEEN ? AND ?
             GROUP BY DATE(created_at), action_type
             ORDER BY DATE(created_at)",
            [$startDate . ' 00:00:00', $endDate . ' 23:59:59']
        );
        
        // Most active articles (most edited/published)
        $activeArticles = $db->fetchAll(
            "SELECT 
                ua.article_id,
                a.title,
                COUNT(DISTINCT CASE WHEN ua.action_type = 'admin_edit' THEN ua.id END) as edits,
                COUNT(DISTINCT CASE WHEN ua.action_type = 'admin_publish' THEN ua.id END) as publishes,
                COUNT(DISTINCT CASE WHEN ua.action_type = 'admin_approve' THEN ua.id END) as approvals,
                MAX(ua.created_at) as last_action
             FROM user_analytics ua
             JOIN articles a ON ua.article_id = a.id
             WHERE ua.action_type IN ('admin_edit', 'admin_publish', 'admin_approve')
                AND ua.created_at BETWEEN ? AND ?
             GROUP BY ua.article_id, a.title
             ORDER BY COUNT(*) DESC
             LIMIT 20",
            [$startDate . ' 00:00:00', $endDate . ' 23:59:59']
        );
        
        echo json_encode([
            'success' => true,
            'activity_summary' => $activitySummary,
            'daily_activity' => $dailyActivity,
            'active_articles' => $activeArticles
        ]);
        
    } catch (Exception $e) {
        http_response_code(500);
        echo json_encode(['success' => false, 'error' => $e->getMessage()]);
    }
}

/**
 * Get user engagement statistics
 */
function getUserEngagement($db) {
    // Check permissions
    if (!isset($_SESSION['user']) || !in_array($_SESSION['user']['role'], ['admin', 'superadmin'])) {
        http_response_code(403);
        echo json_encode(['success' => false, 'error' => 'Unauthorized']);
        return;
    }
    
    $startDate = $_GET['start_date'] ?? date('Y-m-d', strtotime('-30 days'));
    $endDate = $_GET['end_date'] ?? date('Y-m-d');
    
    try {
        // Most engaged users
        $engagedUsers = $db->fetchAll(
            "SELECT 
                u.id,
                u.username,
                u.email,
                u.role,
                COUNT(DISTINCT ua.article_id) as articles_interacted,
                COUNT(DISTINCT CASE WHEN ua.action_type = 'view' THEN ua.article_id END) as articles_viewed,
                COUNT(DISTINCT CASE WHEN ua.action_type = 'bookmark' THEN ua.article_id END) as articles_bookmarked,
                COUNT(DISTINCT CASE WHEN ua.action_type = 'comment' THEN ua.article_id END) as articles_commented,
                COUNT(DISTINCT CASE WHEN ua.action_type = 'star_rating' THEN ua.article_id END) as articles_rated,
                COUNT(*) as total_interactions
             FROM user_analytics ua
             JOIN users u ON ua.user_id = u.id
             WHERE ua.created_at BETWEEN ? AND ?
             AND ua.user_id IS NOT NULL
             GROUP BY u.id, u.username, u.email, u.role
             ORDER BY total_interactions DESC
             LIMIT 50",
            [$startDate . ' 00:00:00', $endDate . ' 23:59:59']
        );
        
        // User retention statistics (returning users)
        $retention = $db->fetchAll(
            "SELECT 
                COUNT(DISTINCT user_id) as total_users,
                COUNT(DISTINCT CASE WHEN visit_count >= 2 THEN user_id END) as returning_users,
                COUNT(DISTINCT CASE WHEN visit_count >= 5 THEN user_id END) as frequent_users,
                COUNT(DISTINCT CASE WHEN visit_count >= 10 THEN user_id END) as power_users
             FROM (
                 SELECT 
                     user_id, 
                     COUNT(DISTINCT DATE(created_at)) as visit_count
                 FROM user_analytics
                 WHERE created_at BETWEEN ? AND ?
                 AND user_id IS NOT NULL
                 GROUP BY user_id
             ) AS user_visits",
            [$startDate . ' 00:00:00', $endDate . ' 23:59:59']
        );
        
        // Engagement by role
        $engagementByRole = $db->fetchAll(
            "SELECT 
                u.role,
                COUNT(DISTINCT u.id) as user_count,
                COUNT(DISTINCT ua.article_id) as articles_interacted,
                ROUND(COUNT(*) / COUNT(DISTINCT u.id), 1) as avg_interactions_per_user
             FROM user_analytics ua
             JOIN users u ON ua.user_id = u.id
             WHERE ua.created_at BETWEEN ? AND ?
             GROUP BY u.role
             ORDER BY avg_interactions_per_user DESC",
            [$startDate . ' 00:00:00', $endDate . ' 23:59:59']
        );
        
        // Engagement over time
        $engagementOverTime = $db->fetchAll(
            "SELECT 
                DATE(created_at) as date,
                COUNT(DISTINCT user_id) as unique_users,
                COUNT(DISTINCT article_id) as unique_articles,
                COUNT(*) as total_interactions
             FROM user_analytics
             WHERE created_at BETWEEN ? AND ?
             GROUP BY DATE(created_at)
             ORDER BY DATE(created_at)",
            [$startDate . ' 00:00:00', $endDate . ' 23:59:59']
        );
        
        echo json_encode([
            'success' => true,
            'engaged_users' => $engagedUsers,
            'retention' => $retention[0] ?? [],
            'engagement_by_role' => $engagementByRole,
            'engagement_over_time' => $engagementOverTime
        ]);
        
    } catch (Exception $e) {
        http_response_code(500);
        echo json_encode(['success' => false, 'error' => $e->getMessage()]);
    }
}
?>