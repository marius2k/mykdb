...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        error_log("=== GET DataTables Request Start ===");
        ...
        error_log("User Role: $userRole, User ID: $currentUserId, Search: '$searchValue'");

        // Build WHERE conditions - DOAR pentru article_versions
        $whereConditions = ["1=1"]; // Start cu o condiție neutră
        $params = [];
        
        // Add role-based filtering
        if ($userRole === 'contributor') {
            $whereConditions[] = "(av.status = 'approved' OR av.status = 'disabled' OR (av.author_id = ? AND av.status IN ('draft', 'pending')))";
        ...
        // Total articles count - DOAR din article_versions
        error_log("=== Getting total count ===");
        $totalStmt = $db->prepare("SELECT COUNT(DISTINCT article_id) FROM article_versions WHERE is_online = 1");
        $totalStmt->execute();
        $totalArticles = $totalStmt->fetchColumn();
        error_log("Total online articles: $totalArticles");

        // Filtered count - DOAR din article_versions
        error_log("=== Getting filtered count ===");
        $filteredQuery = "
        ...
        error_log("Filtered query: $filteredQuery");
        ...
        // Main query - DOAR versiunile online din article_versions
        ...
        error_log("Found " . count($articles) . " online articles");
        ...
        foreach ($articles as &$a) {
            ...
            // Get ALL versions for this article (pentru dropdown-ul Versions)
            ...
            error_log("Processed article ID: " . $a['article_id'] . ", Title: " . $a['title'] . ", Status: " . $a['status'] . ", Version: " . $a['current_version'] . ", Total versions: " . count($a['versions']));
        }
        ...
        error_log("Response structure: draw=$draw, recordsTotal=$totalArticles, recordsFiltered=$filtered, data_count=" . count($articles));
        
        echo json_encode($response);
        error_log("=== GET DataTables Request End ===");
        
    } catch (Exception $e) {
        error_log("=== EXCEPTION in GET ===");
