...
function trackSearchQuery($db, $userId, $data) {
    ...
    error_log('Search Analytics - Inserting search query: ' . $data['query'] . ', results: ' . $resultCount);
    ...
    error_log('Search Analytics - Insert data: ' . json_encode($insertData));
    ...
    try {
        ...
        error_log('Search Analytics - Search query inserted with ID: ' . $insertId);
        
    } catch (Exception $e) {
        error_log('Search Analytics - Error inserting search query: ' . $e->getMessage());
        error_log('Search Analytics - Error trace: ' . $e->getTraceAsString());
        ...
        try {
            if (file_exists(APP_ROOT . '/public/api/bkd_user_analytics.php')) {
                ...
                if (function_exists('trackUserActionDirect')) {
                    ...
                    error_log('Search Analytics - Also tracked in user_activity_analytics');
                }
            }
        } catch (Exception $e) {
            // Don't fail the whole operation if user analytics fails
            error_log('Search Analytics - Warning: Could not track in user_activity_analytics: ' . $e->getMessage());
        }
    }
}
