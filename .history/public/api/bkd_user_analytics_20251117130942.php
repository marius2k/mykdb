...
function getActiveContributors($db) {
    try {
        ...
        // Get contributors stats from admin_activity_analytics and article_versions
        $query = "
            SELECT 
                u.id,
                u.username,
                u.role,
                COUNT(DISTINCT CASE WHEN av.status = 'draft' THEN av.article_id END) as articles_submitted,
                COUNT(DISTINCT CASE WHEN av.status = 'approved' THEN av.article_id END) as articles_approved,
                COUNT(DISTINCT CASE WHEN a.status = 'published' THEN a.id END) as articles_published,
                (
                    COUNT(DISTINCT CASE WHEN av.status = 'draft' THEN av.article_id END) +
                    COUNT(DISTINCT CASE WHEN av.status = 'approved' THEN av.article_id END) +
                    COUNT(DISTINCT CASE WHEN a.status = 'published' THEN a.id END)
                ) as total_activity
            FROM users u
            LEFT JOIN article_versions av ON av.author_id = u.id 
                AND av.created_at BETWEEN ? AND DATE_ADD(?, INTERVAL 1 DAY)
            LEFT JOIN articles a ON a.id = av.article_id AND a.author_id = u.id
    ...
    try {
        ...
        $query = "
            SELECT 
                ...
                u.username,
                u.role,
                COALESCE(SUM(CASE WHEN uaa.action = 'view' THEN 1 ELSE 0 END), 0) as articles_viewed,
                COALESCE(SUM(CASE WHEN uaa.action = 'bookmark' THEN 1 ELSE 0 END), 0) as bookmarks,
                COALESCE(SUM(CASE WHEN uaa.action IN ('star_rating', 'usefulness_rating') THEN 1 ELSE 0 END), 0) as ratings_given,
                (
                    COALESCE(SUM(CASE WHEN uaa.action = 'view' THEN 1 ELSE 0 END), 0) +
                    COALESCE(SUM(CASE WHEN uaa.action = 'bookmark' THEN 3 ELSE 0 END), 0) +
                    COALESCE(SUM(CASE WHEN uaa.action IN ('star_rating', 'usefulness_rating') THEN 2 ELSE 0 END), 0)
                ) as engagement_score
            FROM users u
            LEFT JOIN user_activity_analytics uaa ON uaa.user_id = u.id
                AND uaa.action_date BETWEEN ? AND ?
