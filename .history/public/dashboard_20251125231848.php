    ...
    <div style="width: 50%; overflow: hidden; margin-left: 20px; color: #666; font-size: 14px;">
        <div style="display: inline-block; white-space: nowrap; animation: scroll-left 30s linear infinite;">
            <span><strong style="color: #04709bcc"><?= lang('lang_articles_this_month') ?>:</strong> 
                ...
                <span style="color: #e8f804ff"><?= lang('lang_approved') ?>: <?= $approvedCount ?> </span> 
                <strong style="color: #04709bcc">Total Articles:</strong><?= $db->query("SELECT COUNT(*) FROM articles")->fetchColumn() ?> |
                <strong style="color: #04709bcc">Total Users:</strong> <?= $db->query("SELECT COUNT(*) FROM users")->fetchColumn() ?> |
                <strong style="color: #04709bcc">Online Users:</strong> <?= $onlineUsersCount ?>
            </span>
