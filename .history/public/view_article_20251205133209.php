...
<script>
    ...
    let originalTitle = '';
    let originalContent = '';
    const userLang = '<?= $_SESSION['settings']['language'] ?? 'en' ?>';
        ...
        if (showingOriginal) {
            ...
            try {
                ...
                if (translationCache.success) {
                    ...
                    titleEl.textContent = translationCache.title;
                    contentEl.innerHTML = translationCache.content;
                    btnText.textContent = 'Show Original';
                    showingOriginal = false;
                } else {
                    throw new Error(translationCache.error || 'Translation failed');
                }
            } catch (error) {
    ...
    // Check if translation should be available (detect article language)
    async function checkTranslationAvailability(title, content) {
        try {
            // Simple language detection: check if content contains non-ASCII characters
            // If article is in English and user language is different, show translate button
            const hasNonAscii = /[^\x00-\x7F]/.test(title + content);
            
            // For now, show translate button if user language is not English
            // In production, you'd want to properly detect the article language
            if (userLang !== 'en') {
                const btn = document.getElementById('translate-btn');
                const btnText = document.getElementById('translate-btn-text');
                ...
                const langName = langNames[userLang] || userLang.toUpperCase();
                btnText.textContent = `Translate to ${langName}`;
            }
        } catch (error) {
            ...
            <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <div style="display: flex; gap: 10px; align-items: center;">
                    ...
                    <button id="translate-btn" 
                            style="display: none; padding: 8px 16px; border-radius: 6px; border: 1px solid #ccc; background: white; cursor: pointer; font-size: 0.9rem;"
...
function loadArticle() {
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            checkTranslationAvailability(data.title, data.content);
