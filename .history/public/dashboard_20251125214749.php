...
// Get last login time (from session or database)
$lastLoginStmt = $db->query("SELECT last_login FROM users WHERE id = ?", [$userId]);
$lastLoginData = $lastLoginStmt->fetch();
$lastLogin = $lastLoginData['last_login'] ?? null;
