...
try {
    ...
    // Step 5: Track usefulness ratings
    echo "Step 5: Populating user_activity_analytics with usefulness ratings...\n";
    
    $usefulnessRatings = $db->fetchAll("
        SELECT 
            user_id,
            article_id,
            is_useful as value,
            created_at
        FROM article_usefulness
    ");
    
    echo "Found " . count($usefulnessRatings) . " usefulness ratings...\n";
    
    foreach ($usefulnessRatings as $useful) {
        try {
            $existingUseful = $db->fetchSingle("
                SELECT id FROM user_activity_analytics 
                WHERE user_id = ? 
                AND article_id = ? 
                AND action_type = 'usefulness_rating'
            ", [$useful['user_id'], $useful['article_id']]);
            
            if (!$existingUseful) {
                $db->insert('user_activity_analytics', [
                    'user_id' => $useful['user_id'],
                    'article_id' => $useful['article_id'],
                    'action_type' => 'usefulness_rating',
                    'action_date' => $useful['created_at'],
                    'value' => $useful['value'],
                    'session_id' => 'backfill'
                ]);
                $stats['user_actions']++;
            ...
            echo "  Error processing usefulness: " . $e->getMessage() . "\n";
            $stats['errors']++;
        }
    }
    
    echo "✓ Processed usefulness ratings\n\n";
