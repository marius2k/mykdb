...
1. **Article Analytics** (this document) - Focused on article performance metrics via `articles_analytics.php`
2. **User Analytics** (see `user_analytics.md`) - Focused on user engagement and admin activities via `user_analytics.php`

As part of this separation, the original `analytics.php` file has been renamed to `articles_analytics.php`.
...
The article analytics dashboard is implemented in `public/admin/articles_analytics.php` (renamed from analytics.php) and consists of:
