...
### 2. Null Article IDs in Query Results (Backend)

The top articles query was including records where `article_id` was NULL (from search queries and other non-article actions). This caused:
- Invalid data in the response
- JavaScript errors when trying to display the table
- "Error loading data" alert shown to user
...
### 3. Action Type Mismatch (Frontend)

The JavaScript `displayUserActionsChart()` function was looking for action types that didn't match the database:
- Expected: `'star_rating'`, `'pdf_save'`
- Actual in DB: `'rating'`, `'save_pdf'`

This caused the chart to not display any data even when the API returned successfully.

### 4. Missing Translations (Frontend)

The chart needed translations for action type labels but they were missing from `USER_ANALYTICS_TRANSLATIONS` object.
