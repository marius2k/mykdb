...
const SearchAnalytics = {
    ...
    state: {
        ...
        searchResultCount: 0,
        debounceTimer: null,
        trackingTimer: null, // Separate timer for analytics tracking
    ...
    trackSearch: function(query, resultCount = null) {
        ...
        // Update state immediately (for click tracking correlation)
        ...
        // Clear any existing tracking timer to debounce the actual database save
        if (this.state.trackingTimer) {
            clearTimeout(this.state.trackingTimer);
        }
        
        // Wait for the configured debounce time before actually saving to database
        // This prevents saving "s", "st", "sta", "stac", "stack" as separate records
        this.state.trackingTimer = setTimeout(() => {
            ...
            console.debug('Tracking search (after debounce):', searchData);
            
            // Send data to server
            this.sendData('search_query', searchData);
        }, this.config.debounceTime); // Use the 3000ms debounce time
    },
