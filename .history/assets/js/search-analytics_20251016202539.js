...
const SearchAnalytics = {
    // Configuration
    config: {
        debounceTime: 500, // ms to wait after typing before recording a search (prevents recording every keystroke)
    },
    ...
    attachToSearchForms: function() {
        ...
        document.addEventListener('click', (e) => {
            ...
            const resultLink = e.target.closest('.search-result a, .search-results a, .kb-search-result a');
            ...
            if (resultLink && this.state.lastSearchQuery) {
                this.trackSearchResultClick(
                    ...
                    resultLink.dataset.resultId || this.getPositionInResults(resultLink)
                );
            }
        });
    },
     ...
     */
    trackSearchResultClick: function(url, title, position) {
